...
export function formatPastTimeShort(lang: LangFn, datetime: number | Date) {
  ...
  return formatDateToString(date, noYear);
}

export function formatPastTime(lang: LangFn, datetime: number | Date) {
  const date = typeof datetime === 'number' ? new Date(datetime) : datetime;
  ...
  if (date >= weekAgo) {
    return `${lang(
      `Weekday.Short${WEEKDAYS_FULL[date.getDay()]}`
    )}, ${formatTime(lang, date)}`;
  }

  const noYear = date.getFullYear() === today.getFullYear();

  return `${formatDateToString(date, noYear)}, ${formatTime(lang, date)}`;
}

export function formatFullDate(lang: LangFn, datetime: number | Date) {
  return formatDateToString(datetime, false, 'numeric');
}
...
export function formatMonthAndYear(
  ...
  return formatDateToString(date, false, isShort ? 'short' : 'long', noDate);
}
...
export function formatCountdown(lang: LangFn, msLeft: number) {
  ...
  if (msLeft < 0) {
    ...
    return lang('Duration.Days', { count: days });
  } else if (days < 30) {
    return lang('Duration.Weeks', { count: Math.floor(days / 7) });
  } else if (days < 365) {
    return lang('Duration.Months', { count: Math.floor(days / 30) });
  } else {
    return lang('Duration.Years', { count: Math.floor(days / 365) });
  }
}
...
export function formatLastUpdated(
  ...
  if (seconds < 60) {
    ...
    return lang('LiveLocationUpdated.MinutesAgo', {
      minutes: Math.floor(seconds / 60),
    });
  } else {
    return lang('LiveLocationUpdated.TodayAt', {
      time: formatTime(lang, lastUpdated),
    });
  }
}
...
export function formatTimeDuration(
  ...
  return out
    .map((part) => lang(`Duration.${part.type}`, { count: part.duration }))
    .join(' ');
}
