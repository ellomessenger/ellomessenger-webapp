...
export async function getGroupCall({ call }: { call: Partial<ApiGroupCall> }) {
  const result = await invokeRequest(
    new GramJs.phone.GetGroupCall({
...
export function discardGroupCall({ call }: { call: ApiGroupCall }) {
  return invokeRequest(
    new GramJs.phone.DiscardGroupCall({
      call: buildInputGroupCall(call),
    }),
    true
  );
...
export function editGroupCallParticipant({
  ...
  return invokeRequest(
    new GramJs.phone.EditGroupCallParticipant({
      call: buildInputGroupCall(call),
...
export function editGroupCallTitle({
  ...
  return invokeRequest(
    new GramJs.phone.EditGroupCallTitle({
      title,
...
export async function exportGroupCallInvite({
  ...
  const result = await invokeRequest(
    new GramJs.phone.ExportGroupCallInvite({
...
export async function fetchGroupCallParticipants({
  ...
  const result = await invokeRequest(
    new GramJs.phone.GetGroupParticipants({
      call: buildInputGroupCall(call),
      ids: [],
      sources: [],
      offset: offset || '',
...
export function leaveGroupCall({ call }: { call: ApiGroupCall }) {
  return invokeRequest(
    new GramJs.phone.LeaveGroupCall({
      call: buildInputGroupCall(call),
    }),
    true
  );
...
export async function joinGroupCall({
  ...
  call: ApiGroupCall;
  inviteHash?: string;
  params: JoinGroupCallPayload;
}) {
  const result = await invokeRequest(
    new GramJs.phone.JoinGroupCall({
      call: buildInputGroupCall(call),
  ...
  if (result instanceof GramJs.Updates) {
    const update = result.updates.find(
      (u) => u instanceof GramJs.UpdateGroupCall
    );
...
export async function createGroupCall({ peer }: { peer: ApiChat }) {
  const randomId = generateRandomInt();
  const result = await invokeRequest(
    new GramJs.phone.CreateGroupCall({
...
export function joinGroupCallPresentation({
  ...
  return invokeRequest(
    new GramJs.phone.JoinGroupCallPresentation({
      call: buildInputGroupCall(call),
      params: new GramJs.DataJSON({
        data: JSON.stringify(params),
      }),
...
export function toggleGroupCallStartSubscription({
  ...
  return invokeRequest(
    new GramJs.phone.ToggleGroupCallStartSubscription({
      call: buildInputGroupCall(call),
      subscribed,
    }),
    true,
...
export function leaveGroupCallPresentation({ call }: { call: ApiGroupCall }) {
  return invokeRequest(
    new GramJs.phone.LeaveGroupCallPresentation({
      call: buildInputGroupCall(call),
    }),
    true
  );
...
export function discardCall({
  ...
  return invokeRequest(
    new GramJs.phone.DiscardCall({
      peer: buildInputPhoneCall(call),
      reason: isBusy
        ? new GramJs.PhoneCallDiscardReasonBusy()
        : new GramJs.PhoneCallDiscardReasonHangup(),
    }),
    true
  );
}
...
export async function requestCall({
  ...
  const result = await invokeRequest(
    new GramJs.phone.RequestCall({
      randomId: generateRandomInt(),
      //userId: buildInputPeer(user.id, user.accessHash),
      userId: new GramJs.InputUser({
        userId: buildMtpPeerId(user.id, 'user'),
...
export function setCallRating({
  ...
  return invokeRequest(
    new GramJs.phone.SetCallRating({
      rating,
      peer: buildInputPhoneCall(call),
      comment,
    }),
...
export function receivedCall({ call }: { call: ApiPhoneCall }) {
  return invokeRequest(
    new GramJs.phone.ReceivedCall({
      peer: buildInputPhoneCall(call),
    })
  );
}
...
export function sendSignalingData({
  ...
  return invokeRequest(
    new GramJs.phone.SendSignalingData({
