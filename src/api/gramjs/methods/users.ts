...
import type { OnApiUpdate, ApiUser, ApiChat, ApiSticker } from '../../types';
...
import {
  buildApiUser,
  buildApiUserFromFull,
  buildApiUsersAndStatuses,
...
export async function fetchFullUser({
  ...
  const result = await invokeRequest(
    new GramJs.users.GetFullUser({ id: input })
  );
  ...
  //   localDb.photos[fullInfo.fullUser.personalPhoto.id.toString()] =
  //     fullInfo.fullUser.personalPhoto;
  // }

  // if (fullInfo.fullUser.fallbackPhoto instanceof GramJs.Photo) {
  //   localDb.photos[fullInfo.fullUser.fallbackPhoto.id.toString()] =
  //     fullInfo.fullUser.fallbackPhoto;
  // }
  ...
  if (botInfo?.descriptionDocument instanceof GramJs.Document) {
    localDb.documents[botInfo.descriptionDocument.id.toString()] =
      botInfo.descriptionDocument;
  }

  const userWithFullInfo = buildApiUserFromFull(result);
  ...
  const fullInfo = userWithFullInfo.fullInfo!;
  onUpdate({
    '@type': 'updateUser',
    id,
    user: {
...
export async function fetchCommonChats(
  id: string,
  accessHash?: string,
  maxId?: string
) {
  const commonChats = await invokeRequest(
    new GramJs.messages.GetCommonChats({
      userId: buildInputEntity(id, accessHash) as GramJs.InputUser,
      maxId: maxId
        ? buildMtpPeerId(maxId, getEntityTypeById(maxId))
        : undefined,
...
export async function fetchTopUsers() {
  const topPeers = await invokeRequest(
    new GramJs.contacts.GetTopPeers({
      correspondents: true,
    })
  );
...
export async function importContact({
  ...
  return result?.imported.length
    ? buildApiPeerId(result.imported[0].userId, 'user')
    : undefined;
}
...
export async function deleteContact({
  ...
  const result = await invokeRequest(
    new GramJs.contacts.DeleteContacts({ id: [input] })
  );
...
export async function fetchProfilePhotos(user?: ApiUser, chat?: ApiChat) {
  ...
  return {
    photos: messages
      .map((message) => message.content.action!.photo)
      .filter(Boolean),
    users,
  };
}
...
export function reportSpam(userOrChat: ApiUser | ApiChat) {
  ...
  return invokeRequest(
    new GramJs.messages.ReportSpam({
...
export function updateEmojiStatus(emojiStatus: ApiSticker, expires?: number) {
  return invokeRequest(
    new GramJs.account.UpdateEmojiStatus({
...
function updateLocalDb(
  result:
    | GramJs.photos.Photos
    | GramJs.photos.PhotosSlice
