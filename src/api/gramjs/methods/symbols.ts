...
import {
  buildStickerSet,
  buildStickerSetCovered,
  processStickerPackResult,
  processStickerResult,
...
import {
  DEFAULT_GIF_SEARCH_BOT_USERNAME,
  RECENT_STATUS_LIMIT,
...
export async function fetchCustomEmojiSets({ hash = '0' }: { hash?: string }) {
  // const allStickers = await invokeRequest(
  //   new GramJs.messages.GetEmojiStickers({ hash: BigInt(hash) })
  // );
  // if (
  //   !allStickers ||
  //   allStickers instanceof GramJs.messages.AllStickersNotModified
  // ) {
  //   return undefined;
  // }
...
export async function fetchStickerSets({ hash = '0' }: { hash?: string }) {
  const allStickers = await invokeRequest(
    new GramJs.messages.GetAllStickers({ hash: BigInt(hash) })
  );
...
export async function fetchRecentStickers({ hash = '0' }: { hash?: string }) {
  const result = await invokeRequest(
    new GramJs.messages.GetRecentStickers({ hash: BigInt(hash) })
  );
  ...
  return {
    ...
    stickers: processStickerResult(
      result.stickers.slice(0, RECENT_STICKERS_LIMIT)
    ),
  };
}
...
export async function fetchFavoriteStickers({ hash = '0' }: { hash?: string }) {
  const result = await invokeRequest(
    new GramJs.messages.GetFavedStickers({ hash: BigInt(hash) })
  );
...
export async function fetchFeaturedStickers({ hash = '0' }: { hash?: string }) {
  const result = await invokeRequest(
    new GramJs.messages.GetFeaturedStickers({ hash: BigInt(hash) })
  );

  if (
    !result ||
    result instanceof GramJs.messages.FeaturedStickersNotModified
  ) {
    return undefined;
  }
...
export async function fetchFeaturedEmojiStickers() {
  // const result = await invokeRequest(
  //   new GramJs.messages.GetFeaturedEmojiStickers({ hash: BigInt(0) })
  // );
  // if (
  //   !result ||
  //   result instanceof GramJs.messages.FeaturedStickersNotModified
...
export function removeRecentSticker({ sticker }: { sticker: ApiSticker }) {
...
export async function fetchStickers({
  ...
  // const result = await invokeRequest(
  //   new GramJs.messages.GetStickerSet({
  //     stickerset:
  //       'id' in stickerSetInfo
...
export async function searchStickers({
  ...
  const result = await invokeRequest(
    new GramJs.messages.SearchStickerSets({
      q: query,
      hash: BigInt(hash),
    })
  );
  ...
  if (
    !result ||
    result instanceof GramJs.messages.FoundStickerSetsNotModified
  ) {
    return undefined;
  }
...
export async function fetchSavedGifs({ hash = '0' }: { hash?: string }) {
  const result = await invokeRequest(
    new GramJs.messages.GetSavedGifs({ hash: BigInt(hash) })
  );
...
export async function installStickerSet({
  ...
  const result = await invokeRequest(
    new GramJs.messages.InstallStickerSet({
...
export async function uninstallStickerSet({
  ...
  const result = await invokeRequest(
    new GramJs.messages.UninstallStickerSet({
      stickerset: buildInputStickerSet(stickerSetId, accessHash),
    })
  );
...
export async function searchGifs({
  ...
  if (!inputGifBot) {
    const resolvedPeer = await invokeRequest(
  ...
  const result = await invokeRequest(
    new GramJs.messages.GetInlineBotResults({
      bot: inputGifBot,
...
export async function fetchStickersForEmoji({
  ...
  const result = await invokeRequest(
    new GramJs.messages.GetStickers({
      emoticon: emoji,
      hash: BigInt(hash),
    })
  );
...
export async function fetchEmojiKeywords({
  ...
  const result = await invokeRequest(
    new GramJs.messages.GetEmojiKeywordsDifference({
      langCode: language,
      fromVersion,
    })
  );
...
export async function fetchRecentEmojiStatuses(hash = '0') {
  const result = await invokeRequest(
    new GramJs.account.GetRecentEmojiStatuses({ hash: BigInt(hash) })
  );
