...
export async function fetchChannelStatistics({ chat }: { chat: ApiChat }) {
  const result = await invokeRequest(
    new GramJs.stats.GetBroadcastStats({
      channel: buildInputEntity(
        chat.id,
        chat.accessHash
      ) as GramJs.InputChannel,
    }),
    { dcId: chat.fullInfo!.statisticsDcId }
  );
...
export async function fetchGroupStatistics({ chat }: { chat: ApiChat }) {
  const result = await invokeRequest(
    new GramJs.stats.GetMegagroupStats({
      channel: buildInputEntity(
        chat.id,
        chat.accessHash
      ) as GramJs.InputChannel,
...
export async function fetchMessageStatistics({
  ...
  const result = await invokeRequest(
    new GramJs.stats.GetMessageStats({
      channel: buildInputEntity(
        chat.id,
        chat.accessHash
      ) as GramJs.InputChannel,
...
export async function fetchMessagePublicForwards({
  ...
  const result = await invokeRequest(
    new GramJs.stats.GetMessagePublicForwards({
      channel: buildInputEntity(
        chat.id,
        chat.accessHash
      ) as GramJs.InputChannel,
...
export async function fetchStatisticsAsyncGraph({
  ...
  const result = await invokeRequest(
    new GramJs.stats.LoadAsyncGraph({
      token,
      ...(x && { x: BigInt(x) }),
    }),
    { dcId }
