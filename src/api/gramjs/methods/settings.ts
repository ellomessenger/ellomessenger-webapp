...
export async function sendVerificationCode(payload: {
  ...
  if (newPassword) {
    ...
    onUpdate({
      '@type': 'updateConfirmEmailState',
      type: ConfirmEmailType.confirmChangeEmail,
      expire: parsResult.confirmation_expire,
      email: parsResult.email,
      params: payload,
    });
...
export async function verifyEmailInPassword(payload: {
  email: string;
  password: string;
  tabId?: number;
}) {
  const { email, password, tabId } = payload;
  const sendData = {
    service: 100300,
    method: 100800,
    data: { email, password },
  ...
  onUpdate({
    '@type': 'updateConfirmEmailState',
    type: ConfirmEmailType.DeleteAccount,
    expire: parsResult.confirmation_expire,
    params: { tabId },
  });
}
...
export async function checkUsername(username: string) {
  try {
    const result = await invokeRequest(
      new GramJs.account.CheckUsername({
        username,
      }),
      {
        shouldThrow: true,
...
export async function updateProfilePhoto(
  ...
  const result = await invokeRequest(
    new GramJs.photos.UpdateProfilePhoto({
      id: photoId,
...
export async function uploadProfilePhoto(
  ...
  const result = await invokeRequest(
    new GramJs.photos.UploadProfilePhoto({
      ...(isVideo
        ? { video: inputFile, videoStartTs: videoTs }
        : { file: inputFile }),
      ...(isFallback ? { fallback: true } : undefined),
    })
  );
...
export async function requestBeforeDeletion() {
  const sendData = {
    service: 100300,
    method: 101000,
    data: {},
...
//   const result = await invokeRequest(
//     new GramJs.photos.UploadContactProfilePhoto({
...
export async function deleteProfilePhotos(photos: ApiPhoto[]) {
  ...
  const isDeleted = await invokeRequest(
    new GramJs.photos.DeletePhotos({ id: photoIds }),
    {
      shouldReturnTrue: true,
    }
  );
...
export async function fetchWallpapers() {
  const result = await invokeRequest(
    new GramJs.account.GetWallPapers({ hash: BigInt('0') })
  );
...
export async function uploadWallpaper(file: File) {
  ...
  const result = await invokeRequest(
    new GramJs.account.UploadWallPaper({
      file: inputFile,
      mimeType: file.type,
  ...
  localDb.documents[String(result.document.id)] =
    result.document as GramJs.Document;
...
export async function fetchBlockedContacts() {
  const result = await invokeRequest(
    new GramJs.contacts.GetBlocked({
  ...
  return {
    ...
    chats: result.chats
      ...
      .filter(Boolean),
    blockedIds: result.blocked.map((blocked) =>
      getApiChatIdFromMtpPeer(blocked.peerId)
    ),
    totalCount:
      result instanceof GramJs.contacts.BlockedSlice
        ? result.count
...
export async function fetchAuthorizations() {
  ...
  return {
    authorizations: buildCollectionByKey(
      result.authorizations.map(buildApiSession),
      'hash'
    ),
...
export async function fetchWebAuthorizations() {
  ...
  return {
    users: result.users.map(buildApiUser).filter(Boolean),
    webAuthorizations: buildCollectionByKey(
      result.authorizations.map(buildApiWebSession),
      'hash'
    ),
  };
}
...
export function terminateWebAuthorization(hash: string) {
  return invokeRequest(
    new GramJs.account.ResetWebAuthorization({ hash: BigInt(hash) })
  );
}
...
export async function fetchNotificationExceptions() {
  const result = await invokeRequest(
    new GramJs.account.GetNotifyExceptions({
      compareSound: true,
    }),
    {
...
export async function fetchNotificationSettings() {
  const [
    ...
    invokeRequest(
      new GramJs.account.GetNotifySettings({
  ...
  return {
    ...
    hasPrivateChatsNotifications: !(
      ...
      (typeof privateMuteUntil === 'number' &&
        getServerTime() < privateMuteUntil)
    ),
    hasPrivateChatsMessagePreview: privateShowPreviews,
    hasGroupNotifications: !(
      groupSilent ||
      (typeof groupMuteUntil === 'number' && getServerTime() < groupMuteUntil)
    ),
    hasGroupMessagePreview: groupShowPreviews,
    hasBroadcastNotifications: !(
      broadcastSilent ||
      (typeof broadcastMuteUntil === 'number' &&
...
export function updateContactSignUpNotification(isSilent: boolean) {
  return invokeRequest(
    new GramJs.account.SetContactSignUpNotification({ silent: isSilent })
  );
}
...
export async function fetchLanguages(): Promise<ApiLanguage[] | undefined> {
  const result = await invokeRequest(
    new GramJs.langpack.GetLanguages({
...
export async function fetchLangPack({
  ...
  const results = await Promise.all(
    sourceLangPacks.map((langPack) => {
      return invokeRequest(
        new GramJs.langpack.GetLangPack({
          langPack,
          langCode: BETA_LANG_CODES.includes(langCode)
            ? `${langCode}-raw`
            : langCode,
        })
      );
  ...
  const collections = results.filter(Boolean).map((result) => {
    return buildCollectionByKey(
      result.strings.map<ApiLangString>(omitVirtualClassFields),
      'key'
    );
  });
...
export async function fetchLangStrings({
  ...
  const result = await invokeRequest(
    new GramJs.langpack.GetStrings({
      langPack,
      langCode: BETA_LANG_CODES.includes(langCode)
        ? `${langCode}-raw`
        : langCode,
...
export function unregisterDevice(token: string) {
  return invokeRequest(
    new GramJs.account.UnregisterDevice({
      tokenType: 10,
      otherUids: [],
      token,
    })
...
export async function setPrivacySettings(
  ...
  if (rules.allowedUsers) {
    privacyRules.push(
      new GramJs.InputPrivacyValueAllowUsers({
        users: rules.allowedUsers.map(
          ({ id, accessHash }) =>
            buildInputEntity(id, accessHash) as GramJs.InputUser
        ),
      })
    );
  }
  if (rules.allowedChats) {
    privacyRules.push(
      new GramJs.InputPrivacyValueAllowChatParticipants({
        chats: rules.allowedChats.map(
          ({ id }) => buildInputEntity(id) as BigInt.BigInteger
        ),
      })
    );
  }
  if (rules.blockedUsers) {
    privacyRules.push(
      new GramJs.InputPrivacyValueDisallowUsers({
  ...
  if (rules.blockedChats) {
    privacyRules.push(
      new GramJs.InputPrivacyValueDisallowChatParticipants({
  ...
  const result = await invokeRequest(
    new GramJs.account.SetPrivacy({ key, rules: privacyRules })
  );
...
export async function fetchAppConfig(
  hash?: number
): Promise<ApiAppConfig | undefined> {
  const result = await invokeRequest(new GramJs.help.GetAppConfig({ hash }));

  if (!result) return undefined;

  const { value, hash: resultHash } = result;

  return buildAppConfig(result, resultHash);
}
...
function updateLocalDb(
  result:
    | GramJs.account.PrivacyRules
    | GramJs.contacts.Blocked
    | GramJs.contacts.BlockedSlice
    | GramJs.Updates
    | GramJs.UpdatesCombined
) {
  addEntitiesWithPhotosToLocalDb(result.users);
...
export async function fetchCountryList({
  ...
  const countryList = await invokeRequest(
    new GramJs.help.GetCountriesList({
      langCode,
    })
  );
...
export async function fetchGlobalPrivacySettings() {
  const result = await invokeRequest(
    new GramJs.account.GetGlobalPrivacySettings()
  );
  ...
  return {
    shouldArchiveAndMuteNewNonContact: Boolean(
      result.archiveAndMuteNewNoncontactPeers
    ),
  };
}
...
export async function updateGlobalPrivacySettings({
  ...
  const result = await invokeRequest(
    new GramJs.account.SetGlobalPrivacySettings({
...
export async function fetchCategoriesForChannel() {
  let sendData = {
    service: 100400,
    method: 100100,
  };
...
export async function fetchGenresForChannel() {
  let sendData = {
    service: 100400,
    method: 100200,
  };
...
export async function fetchPaidSubscriptions(filter_type: number) {
  let sendData = {
    service: 100600,
    method: 100100,
    data: { filter_type },
...
//   if (chatId) {
//     return invokeRequest(
//       new GramJs.channels.ToggleUsername({
...
//   return invokeRequest(
//     new GramJs.account.ToggleUsername({
//       username,
//       active: isActive,
//     })
//   );
...
// export function reorderUsernames({
//   chatId,
//   accessHash,
//   usernames,
// }: {
...
//   if (chatId) {
//     return invokeRequest(
//       new GramJs.channels.ReorderUsernames({
//         channel: buildInputEntity(chatId, accessHash) as GramJs.InputChannel,
//         order: usernames,
//       })
//     );
...
//   return invokeRequest(
//     new GramJs.account.ReorderUsernames({
//       order: usernames,
//     })
//   );
// }
