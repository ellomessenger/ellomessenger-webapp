...
export async function fetchInlineBotResults({
  ...
  return {
    ...
    nextOffset: getInlineBotResultsNextOffset(
...
export async function sendInlineBotResult({
  ...
  await invokeRequest(
    new GramJs.messages.SendInlineBotResult({
      clearDraft: true,
      randomId,
...
export async function requestWebView({
  ...
  const result = await invokeRequest(
    new GramJs.messages.RequestWebView({
...
export async function requestSimpleWebView({
  ...
  const result = await invokeRequest(
    new GramJs.messages.RequestSimpleWebView({
      url,
      bot: buildInputPeer(bot.id, bot.accessHash),
      themeParams: theme ? buildInputThemeParams(theme) : undefined,
      platform: 'weba',
...
export function prolongWebView({
  ...
  return invokeRequest(
    new GramJs.messages.ProlongWebView({
      silent: isSilent || undefined,
      peer: buildInputPeer(peer.id, peer.accessHash),
      bot: buildInputPeer(bot.id, bot.accessHash),
      queryId: BigInt(queryId),
...
export async function sendWebViewData({
  ...
  const randomId = generateRandomBigInt();
  await invokeRequest(
    new GramJs.messages.SendWebViewData({
      bot: buildInputPeer(bot.id, bot.accessHash),
      buttonText,
      data,
      randomId,
...
export async function loadAttachBots({ hash }: { hash?: string }) {
  const result = await invokeRequest(
    new GramJs.messages.GetAttachMenuBots({
...
export async function loadAttachBot({ bot }: { bot: ApiUser }) {
  const result = await invokeRequest(
    new GramJs.messages.GetAttachMenuBot({
      bot: buildInputPeer(bot.id, bot.accessHash),
    })
  );
...
export function toggleAttachBot({
  ...
  return invokeRequest(
    new GramJs.messages.ToggleBotInAttachMenu({
      bot: buildInputPeer(bot.id, bot.accessHash),
...
export async function requestBotUrlAuth({
  ...
  const result = await invokeRequest(
    new GramJs.messages.RequestUrlAuth({
      peer: buildInputPeer(chat.id, chat.accessHash),
      buttonId,
      msgId: messageId,
    })
...
export async function acceptBotUrlAuth({
  ...
  const result = await invokeRequest(
    new GramJs.messages.AcceptUrlAuth({
      peer: buildInputPeer(chat.id, chat.accessHash),
      msgId: messageId,
      buttonId,
      writeAllowed: isWriteAllowed || undefined,
...
function processInlineBotResult(
  queryId: string,
  results: GramJs.TypeBotInlineResult[]
) {
...
export async function fetchChatBotList() {
  const sendData = {
    service: 100700,
    method: 100800,
  };
