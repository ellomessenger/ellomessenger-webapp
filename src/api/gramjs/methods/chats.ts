...
import { isChatSubscription } from '../../../global/helpers';
...
export async function fetchChats({
  ...
  const lastMessagesByChatId = buildCollectionByKey(
    (resultPinned ? resultPinned.messages : [])
  ...
  dialogs.forEach((dialog) => {
    ...
    if (
      ...
      lastLocalServiceMessage &&
      (!lastMessagesByChatId[chat.id] ||
        lastLocalServiceMessage.date > lastMessagesByChatId[chat.id].date)
    ) {
    ...
    if (dialog.draft) {
      const { formattedText, replyingToId } =
        buildMessageDraft(dialog.draft) || {};
      if (formattedText) {
...
export async function searchChats(payload: ISearchQuery) {
  const result = await invokeRequest(
  ...
  const allChats = result.chats
    .concat(result.users.filter((user) => !isCurrentUser(user)))
  ...
  return {
    ...
    userStatusesById: buildApiUsersAndStatuses(result.users).userStatusesById,
  };
}
...
export async function fetchChat({
  ...
  if (type === 'self' || type === 'user') {
    ...
    if (!result || !result.length) {
      if (user || userId) {
        const fullInfo = await invokeRequest(
          new GramJs.users.GetFullUser({
            id: user
              ? (buildInputEntity(user.id, user.accessHash) as GramJs.InputUser)
              : userId
              ? (buildInputEntity(userId) as GramJs.InputUser)
              : new GramJs.InputUserEmpty(),
          })
        );

        if (!fullInfo) return undefined;
        result = fullInfo.users;
        const newUser = buildApiUser(fullInfo.users[0]);
        if (newUser) {
          onUpdate({
            '@type': 'updateUser',
            id: newUser.id,
...
export async function requestChatUpdate({
  ...
  const peerEntity =
    peersByKey[
      id === SERVICE_NOTIFICATIONS_USER_ID
        ? `user${result.users[0].id}`
        : getPeerKey(dialog.peer)
    ];
  if (!peerEntity) {
    return;
  }
  ...
  if (!noLastMessage && lastMessage && lastMessage.chatId === chatUpdate.id) {
    onUpdate({
...
export function saveDraft({
  ...
  return invokeRequest(
    new GramJs.messages.SaveDraft({
      ...
      ...(threadId && { replyToMsgId: threadId }),
    })
  );
}
...
async function getFullChannelInfo(
  ...
  const { members, users, userStatusesById } =
  ...
  return {
    fullInfo: {
      ...
      linkedChatId: linkedChatId
        ? buildApiPeerId(linkedChatId, 'chat')
        : undefined,
      botCommands,
      enabledReactions: buildApiChatReactions(availableReactions),
...
export async function createChannel(
  ...
  const result = await invokeRequest(
    new GramJs.channels.CreateChannel({
      ...(!megagroup && { broadcast: true }),
      title,
      about,
      payType,
      cost: payType ? cost : undefined,
      startDate,
      endDate,
      category,
      country,
...
export function paidSubscribe(payload: { peer_id: number; peer_type: number }) {
  let sendData = {
    service: 100600,
    method: 100200,
    data: payload,
...
export function unSubscribe(payload: { peer_id: number; peer_type: number }) {
  let sendData = {
    service: 100600,
    method: 100300,
    data: payload,
...
export async function fetchChatFolders() {
  ...
  const defaultFolderPosition = result.findIndex(
...
export async function editCreator({
  ...
  const userId = buildInputEntity(user.id, user.accessHash) as GramJs.InputUser;
  const password = new GramJs.InputCheckPasswordEmpty();

  await invokeRequest(
    new GramJs.channels.EditCreator({
      channel,
      userId,
      password,
    })
...
export function setDiscussionGroup({
  ...
  //buildInputPeer(channel.id, channel.accessHash)
  //buildInputPeer(chat.id, chat.accessHash)
  return invokeRequest(
    new GramJs.channels.SetDiscussionGroup({
      broadcast: new GramJs.InputChannel({
        channelId: buildMtpPeerId(channel.id, 'channel'),
        accessHash: BigInt(channel.accessHash!),
      }),
      group: chat
        ? new GramJs.InputChannel({
            channelId: buildMtpPeerId(chat.id, 'channel'),
...
export async function openChatByInvite(hash: string) {
  ...
  } else if (result instanceof GramJs.ChatInviteUser) {
    const newUser = buildApiUser(result.user);
    chat = buildApiChatFromPreview(result.user);
...
export function toggleForum({
  ...
  return invokeRequest(
    ...
    { shouldReturnTrue: true, shouldThrow: true }
  );
}
...
> {
  ...
  const chats = result.chats
    .map((c: GramJs.TypeChat | GramJs.TypeUser) => buildApiChatFromPreview(c))
    .filter(Boolean);
  const draftsById = result.topics.reduce(
    (
      acc: (
        | {
            formattedText: ApiFormattedText | undefined;
            replyingToId: number | undefined;
            date: number;
          }
        | undefined
      )[],
      topic: { draft: GramJs.TypeDraftMessage; id: string | number }
    ) => {
  ...
  const readInboxMessageIdByTopicId = result.topics.reduce(
    (acc: number[], topic: { readInboxMaxId: any; id: string | number }) => {
