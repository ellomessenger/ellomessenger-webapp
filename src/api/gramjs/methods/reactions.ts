...
export async function getAvailableReactions() {
  const result = await invokeRequest(
    new GramJs.messages.GetAvailableReactions({})
  );

  if (
    !result ||
    result instanceof GramJs.messages.AvailableReactionsNotModified
  ) {
    return undefined;
  }
  ...
  result.reactions.forEach((reaction) => {
    ...
    if (reaction.selectAnimation instanceof GramJs.Document) {
      localDb.documents[String(reaction.selectAnimation.id)] =
        reaction.selectAnimation;
    }
    if (reaction.aroundAnimation instanceof GramJs.Document) {
      localDb.documents[String(reaction.aroundAnimation.id)] =
        reaction.aroundAnimation;
    }
    if (reaction.appearAnimation instanceof GramJs.Document) {
      localDb.documents[String(reaction.appearAnimation.id)] =
...
export function sendReaction({
  ...
  return invokeRequest(
    new GramJs.messages.SendReaction({
...
export function fetchMessageReactions({
  ...
  return invokeRequest(
    new GramJs.messages.GetMessagesReactions({
      id: ids,
      peer: buildInputPeer(chat.id, chat.accessHash),
    }),
    {
...
export async function fetchMessageReactionsList({
  ...
  const result = await invokeRequest(
    new GramJs.messages.GetMessageReactionsList({
      peer: buildInputPeer(chat.id, chat.accessHash),
      id: messageId,
...
export function setDefaultReaction({ reaction }: { reaction: ApiReaction }) {
  return invokeRequest(
    new GramJs.messages.SetDefaultReaction({
...
export async function fetchTopReactions({ hash = '0' }: { hash?: string }) {
  const result = await invokeRequest(
    new GramJs.messages.GetTopReactions({
...
export async function fetchRecentReactions({ hash = '0' }: { hash?: string }) {
  const result = await invokeRequest(
    new GramJs.messages.GetRecentReactions({
