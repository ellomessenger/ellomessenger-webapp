...
export async function checkChatUsername({ username }: { username: string }) {
  try {
    const result = await invokeRequest(
      new GramJs.channels.CheckUsername({
...
export async function checkEmail({ email }: { email: string }) {
  const result = await invokeRequest(
    new GramJs.account.AuthCheckAvailabilityEmail({ email })
  );
  return result;
}
...
export async function updatePrivateLink({
  ...
  const result = await invokeRequest(
    new GramJs.messages.ExportChatInvite({
...
export async function fetchExportedChatInvites({
  ...
  const exportedInvites = await invokeRequest(
    new GramJs.messages.GetExportedChatInvites({
  ...
  const invites = exportedInvites.invites
    .filter(
      (invite): invite is GramJs.ChatInviteExported =>
        invite instanceof GramJs.ChatInviteExported
    )
...
export async function editExportedChatInvite({
  ...
  const invite = await invokeRequest(
    new GramJs.messages.EditExportedChatInvite({
      link,
      peer: buildInputPeer(peer.id, peer.accessHash),
      expireDate,
  ...
  if (
    invite instanceof GramJs.messages.ExportedChatInvite &&
  ...
  if (
    invite instanceof GramJs.messages.ExportedChatInviteReplaced &&
    invite.invite instanceof GramJs.ChatInviteExported &&
...
export async function deleteExportedChatInvite({
  ...
  const result = await invokeRequest(
    new GramJs.messages.DeleteExportedChatInvite({
      peer: buildInputPeer(peer.id, peer.accessHash),
      link,
    })
  );
...
export async function deleteRevokedExportedChatInvites({
  ...
  const result = await invokeRequest(
    new GramJs.messages.DeleteRevokedExportedChatInvites({
      peer: buildInputPeer(peer.id, peer.accessHash),
      adminId: buildInputEntity(admin.id, admin.accessHash) as GramJs.InputUser,
    })
  );
...
export async function fetchChatInviteImporters({
  ...
  const result = await invokeRequest(
    new GramJs.messages.GetChatInviteImporters({
      peer: buildInputPeer(peer.id, peer.accessHash),
      link,
      offsetDate,
  ...
  return {
    importers: result.importers.map((importer) =>
      buildChatInviteImporter(importer)
    ),
...
export function hideChatJoinRequest({
  ...
  return invokeRequest(
    new GramJs.messages.HideChatJoinRequest({
      peer: buildInputPeer(peer.id, peer.accessHash),
...
export function hideAllChatJoinRequests({
  ...
  return invokeRequest(
    new GramJs.messages.HideAllChatJoinRequests({
      peer: buildInputPeer(peer.id, peer.accessHash),
      approved: isApproved || undefined,
      link,
    }),
...
export function hideChatReportPanel(chat: ApiChat) {
  ...
  return invokeRequest(
    new GramJs.messages.HidePeerSettingsBar({
