...
import {
  buildApiEmojiInteraction,
  buildStickerSet,
} from './apiBuilders/symbols';
import { buildApiBotMenuButton } from './apiBuilders/bots';
import { UpdatePts } from './updates/UpdatePts';

export type Update =
  | ((GramJs.TypeUpdate | GramJs.TypeUpdates) & {
...
export function dispatchThreadInfoUpdates(
  messages: (GramJs.TypeMessage | undefined)[]
) {
  const threadInfoUpdates = compact(messages)
    .map(buildApiThreadInfoFromMessage)
    .filter(Boolean);
...
export function dispatchUserAndChatUpdates(
  entities: (GramJs.TypeUser | GramJs.TypeChat)[]
) {
  entities
  ...
  entities
    .filter(
      (e) =>
        e instanceof GramJs.Chat ||
        e instanceof GramJs.ChatForbidden ||
        e instanceof GramJs.Channel ||
        e instanceof GramJs.ChannelForbidden
    )
...
export function updater(update: Update) {
  ...
  if (update instanceof connection.UpdateServerTimeOffset) {
    ...
    if (update instanceof GramJs.UpdateShortChatMessage) {
      ...
      if (
        (update.message instanceof GramJs.Message &&
          isMessageWithMedia(update.message)) ||
    ...
    if (update instanceof GramJs.UpdateNewScheduledMessage) {
      onUpdate({
        '@type': sentMessageIds.has(message.id)
      ...
      if (action instanceof GramJs.MessageActionPaymentSent) {
        ...
        if (avatarHash) {
          onUpdate({
            ...
            ...(action.photo instanceof GramJs.Photo && {
              newProfilePhoto: buildApiPhoto(action.photo),
            }),
          });
        }
        ...
        if (
          update._entities &&
          update._entities.some(
            (e): e is GramJs.User =>
              e instanceof GramJs.User &&
              Boolean(e.self) &&
              e.id === action.userId
          )
        ) {
        ...
        //   } = replyTo || {};
        //   const topicId = !isTopicReply
        //     ? GENERAL_TOPIC_ID
        //     : replyToTopId || replyToMsgId || GENERAL_TOPIC_ID;
    ...
    if (update.popup) {
      ...
      if (isMessageWithMedia(update)) {
        addMessageToLocalDb(
          buildMessageFromUpdate(message.id, message.chatId, update)
        );
      }
    ...
    if (update.action instanceof GramJs.SendMessageEmojiInteraction) {
      onUpdate({
        ...
        interaction: buildApiEmojiInteraction(
          JSON.parse(update.action.interaction.data)
        ),
      });
    } else {
      onUpdate({
    ...
    const channel = _entities.find(
      (e): e is GramJs.Channel | GramJs.ChannelForbidden =>
    ...
    const user =
      updatedUser?.mutualContact && !updatedUser.self
        ? pick(update, [])
    ...
    const {
      notifySettings: { showPreviews, silent, muteUntil },
      peer: { className },
    } = update;
      ...
      isSilent: Boolean(
        silent ||
          (typeof muteUntil === 'number' &&
            Date.now() + getServerTimeOffset() * 1000 < muteUntil * 1000)
      ),
    ...
    onUpdate({
      ...
      chatId: getApiChatIdFromMtpPeer(update.peer),
      recentRequesterIds: update.recentRequesters.map((id) =>
        buildApiPeerId(id, 'user')
      ),
    ...
    const params =
      typeof update === 'object' && 'className' in update
        ? update.className
        : update;
