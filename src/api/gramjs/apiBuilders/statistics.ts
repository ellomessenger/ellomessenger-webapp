...
export function buildChannelStatistics(
  stats: GramJs.stats.BroadcastStats
): ApiChannelStatistics {
  return {
    // Graphs
    ...
    viewsBySourceGraph: (stats.viewsBySourceGraph as GramJs.StatsGraphAsync)
      .token,
    newFollowersBySourceGraph: (
      stats.newFollowersBySourceGraph as GramJs.StatsGraphAsync
    ).token,
    interactionsGraph: (stats.interactionsGraph as GramJs.StatsGraphAsync)
      .token,
...
export function buildGroupStatistics(
  stats: GramJs.stats.MegagroupStats
): ApiGroupStatistics {
  return {
    // Graphs
    growthGraph: buildGraph(stats.growthGraph),
...
export function buildMessageStatistics(
  stats: GramJs.stats.MessageStats
): ApiMessageStatistics {
  return {
...
export function buildMessagePublicForwards(
  ...
  return result.messages.map((message) => {
    const peerId = buildApiPeerId(
      (message.peerId as GramJs.PeerChannel).channelId,
      'channel'
    );
    const channel = result.chats.find(
      (p) => buildApiPeerId(p.id, 'channel') === peerId
    );
...
export function buildGraph(
  ...
  return {
    ...
    labels: x.slice(1),
    hideCaption: !(data.subchart && data.subchart.show),
    hasSecondYAxis,
...
function calculateMinimapRange(range: Array<number>, values: Array<number>) {
  ...
  return {
    minimapRange: { begin, end },
    labelFromIndex: minIndex,
    labelToIndex: maxIndex,
  };
}

function buildStatisticsOverview({
  current,
  previous,
}: GramJs.StatsAbsValueAndPrev): StatisticsOverviewItem {
...
function buildStatisticsPercentage(
  data: GramJs.StatsPercentValue
): StatisticsOverviewPercentage {
  return {
...
function getOverviewPeriod(
  data: GramJs.StatsDateRangeDays
): StatisticsOverviewPeriod {
  return {
