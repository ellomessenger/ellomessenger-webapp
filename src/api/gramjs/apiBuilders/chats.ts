...
function buildApiChatFieldsFromPeerEntity(
  ...
  const accessHash =
    'accessHash' in peerEntity && String(peerEntity.accessHash);
  const hasVideoAvatar =
    'photo' in peerEntity &&
    peerEntity.photo &&
    'hasVideo' in peerEntity.photo &&
    peerEntity.photo.hasVideo;
  const avatarHash =
    'photo' in peerEntity &&
    peerEntity.photo &&
    buildAvatarHash(peerEntity.photo);
  const isSignaturesShown = Boolean(
    'signatures' in peerEntity && peerEntity.signatures
  );
  ...
  const isJoinToSend = Boolean(
    'joinToSend' in peerEntity && peerEntity.joinToSend
  );
  const isJoinRequest = Boolean(
    'joinRequest' in peerEntity && peerEntity.joinRequest
  );
  ...
  return {
    ...
    ...('participantsCount' in peerEntity &&
      peerEntity.participantsCount !== undefined && {
    ...
    isJoinRequest,
    isForum,
    ...('payType' in peerEntity && { payType: peerEntity.payType }),
    ...('cost' in peerEntity && { cost: peerEntity.cost }),
    ...('startDate' in peerEntity && {
      startDate: String(peerEntity.startDate),
    }),
    ...('endDate' in peerEntity && { endDate: String(peerEntity.endDate) }),
    ...('category' in peerEntity && { category: peerEntity.category }),
    ...('genre' in peerEntity && { genre: peerEntity.genre }),
    ...('subGenre' in peerEntity && { subGenre: peerEntity.subGenre }),
    ...('country' in peerEntity && { country: peerEntity.country }),
    ...('adult' in peerEntity && { adult: peerEntity.adult }),
  };
}
...
export function buildApiChatFromDialog(
  ...
  const isMuted =
    silent || (typeof muteUntil === 'number' && getServerTime() < muteUntil);
...
function buildApiChatPermissions(
  peerEntity: GramJs.TypeUser | GramJs.TypeChat
): {
  ...
  return {
    adminRights: peerEntity.adminRights
      ? omitVirtualClassFields(peerEntity.adminRights)
      : undefined,
    currentUserBannedRights:
      'bannedRights' in peerEntity && peerEntity.bannedRights
...
function buildApiChatRestrictionReason(
  ...
  const targetReason = restrictionReasons.find(
...
export function buildApiChatFromPreview(
  ...
  if (
    preview instanceof GramJs.ChatEmpty ||
    preview instanceof GramJs.UserEmpty
  ) {
    return undefined;
  }
  ...
  return {
    id: buildApiPeerId(
      preview.id,
      preview instanceof GramJs.User ? 'user' : 'chat'
    ),
    type: getApiChatTypeFromPeerEntity(preview),
    title:
      preview instanceof GramJs.User ? getUserName(preview) : preview.title,
    ...buildApiChatFieldsFromPeerEntity(preview, isSupport),
  };
}

export function getApiChatTypeFromPeerEntity(
  peerEntity: GramJs.TypeChat | GramJs.TypeUser
) {
  if (
    peerEntity instanceof GramJs.User ||
    peerEntity instanceof GramJs.UserEmpty
  ) {
    ...
    return peerEntity.megagroup
      ? 'chatTypeSuperGroup'
      : peerEntity.payType === 1
      ? 'chatTypeChannelCourse'
      : peerEntity.payType === 2
...
export function getApiChatTitleFromMtpPeer(
  peer: GramJs.TypePeer,
  peerEntity: GramJs.User | GramJs.Chat
) {
  if (isPeerUser(peer)) {
...
export function buildAvatarHash(
  photo: GramJs.TypeUserProfilePhoto | GramJs.TypeChatPhoto
) {
  if ('photoId' in photo) {
...
export function buildChatMember(
  ...
  const userId =
    member instanceof GramJs.ChannelParticipantBanned ||
    member instanceof GramJs.ChannelParticipantLeft
  ...
  return {
    userId,
    inviterId:
      'inviterId' in member && member.inviterId
        ? buildApiPeerId(member.inviterId as BigInt.BigInteger, 'user')
        : undefined,
    joinedDate: 'date' in member ? member.date : undefined,
    kickedByUserId:
      'kickedBy' in member && member.kickedBy
        ? buildApiPeerId(member.kickedBy, 'user')
        : undefined,
    promotedByUserId:
      'promotedBy' in member && member.promotedBy
        ? buildApiPeerId(member.promotedBy, 'user')
        : undefined,
    bannedRights:
      'bannedRights' in member
        ? omitVirtualClassFields(member.bannedRights)
        : undefined,
    adminRights:
      'adminRights' in member
        ? omitVirtualClassFields(member.adminRights)
        : undefined,
    customTitle: 'rank' in member ? member.rank : undefined,
    ...((member instanceof GramJs.ChannelParticipantAdmin ||
      member instanceof GramJs.ChatParticipantAdmin) && {
      isAdmin: true,
    }),
    ...((member instanceof GramJs.ChannelParticipantCreator ||
      member instanceof GramJs.ChatParticipantCreator) && {
      isOwner: true,
    }),
  };
}
...
export function buildChatTypingStatus(
  update:
    | GramJs.UpdateUserTyping
    | GramJs.UpdateChatUserTyping
    | GramJs.UpdateChannelUserTyping
) {
  let action: string = '';
  ...
  return {
    ...
    ...(!(update instanceof GramJs.UpdateUserTyping) && {
      userId: getApiChatIdFromMtpPeer(update.fromId),
    }),
...
export function buildApiChatFolder(filter: GramJs.DialogFilter): ApiChatFolder {
  ...
  //   return {
  //     ...pickTruthy(filter, ['id', 'title', 'emoticon']),
  //     excludedChatIds: [],
  //     includedChatIds: filter.includePeers
  //       .map(getApiChatIdFromMtpPeer)
  //       .filter(Boolean),
  //     pinnedChatIds: filter.pinnedPeers
  //       .map(getApiChatIdFromMtpPeer)
  //       .filter(Boolean),
  ...
  return {
    ...
    includedChatIds: filter.includePeers
      .map(getApiChatIdFromMtpPeer)
      .filter(Boolean),
    excludedChatIds: filter.excludePeers
      .map(getApiChatIdFromMtpPeer)
      .filter(Boolean),
  };
}
...
export function buildApiChatBotCommands(botInfos: GramJs.BotInfo[]) {
  return botInfos.reduce((botCommands, botInfo) => {
    ...
    if (botInfo.commands) {
      botCommands = botCommands.concat(
        botInfo.commands.map((mtpCommand) => ({
          botId,
...
export function buildApiExportedInvite(
  invite: GramJs.ChatInviteExported
): ApiExportedInvite {
  const {
    revoked,
    date,
...
export function buildChatInviteImporter(
  importer: GramJs.ChatInviteImporter
): ApiChatInviteImporter {
  const { userId, date, about, requested } = importer;
  return {
...
export function buildApiChatReactions(
  chatReactions?: GramJs.TypeChatReactions
...
export function buildApiSendAsPeerId(
  sendAs: GramJs.SendAsPeer
): ApiSendAsPeerId {
  return {
...
export function buildApiTopic(
  forumTopic: GramJs.TypeForumTopic
): ApiTopic | undefined {
...
export function buildApiChatlistInvite(
  invite: GramJs.chatlists.TypeChatlistInvite | undefined,
  slug: string
  ...
  if (invite instanceof GramJs.chatlists.ChatlistInviteAlready) {
    return {
      ...
      missingPeerIds: invite.missingPeers
        .map(getApiChatIdFromMtpPeer)
        .filter(Boolean),
      alreadyPeerIds: invite.alreadyPeers
        .map(getApiChatIdFromMtpPeer)
        .filter(Boolean),
    };
  }
