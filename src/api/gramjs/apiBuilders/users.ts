...
export function buildApiUserFromFull(
  mtpUserFull: GramJs.users.UserFull
): ApiUser {
  const {
    fullUser: {
      about,
  ...
  return {
    ...user,
    fullInfo: {
      ...(profilePhoto instanceof GramJs.Photo && {
        profilePhoto: buildApiPhoto(profilePhoto),
      }),
      // ...(fallbackPhoto instanceof GramJs.Photo && {
      //   fallbackPhoto: buildApiPhoto(fallbackPhoto),
      // }),
      // ...(personalPhoto instanceof GramJs.Photo && {
      //   personalPhoto: buildApiPhoto(personalPhoto),
      // }),
      bio: about,
      gender,
      date,
      ...
      ...(premiumGifts && {
        premiumGifts: premiumGifts.map((gift) =>
          buildApiPremiumGiftOption(gift)
        ),
      }),
...
export function buildApiUser(mtpUser: GramJs.TypeUser): ApiUser | undefined {
  ...
  const hasVideoAvatar =
    mtpUser.photo instanceof GramJs.UserProfilePhoto
  ...
  const emojiStatus = mtpUser.emojiStatus
    ? buildApiUserEmojiStatus(mtpUser.emojiStatus)
    : undefined;
  ...
  return {
    ...
    hasVideoAvatar,
    ...(mtpUser.bot &&
      mtpUser.botInlinePlaceholder && {
        botPlaceholder: mtpUser.botInlinePlaceholder,
      }),
    ...(mtpUser.bot &&
      mtpUser.botAttachMenu && { isAttachBot: mtpUser.botAttachMenu }),
...
export function buildApiUserStatus(
  mtpStatus?: GramJs.TypeUserStatus
): ApiUserStatus {
...
export function buildApiUserEmojiStatus(
  mtpEmojiStatus: GramJs.TypeEmojiStatus
): ApiEmojiStatus | undefined {
...
export function buildApiPremiumGiftOption(
  option: GramJs.TypePremiumGiftOption
): ApiPremiumGiftOption {
  const { months, currency, amount, botUrl } = option;
