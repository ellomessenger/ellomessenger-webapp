...
function buildEmojiSounds(appConfig: GramJsAppConfig) {
  ...
  return emojies_sounds
    ? Object.keys(emojies_sounds).reduce((acc: Record<string, string>, key) => {
        ...
        localDb.documents[l.id] = new GramJs.Document({
          ...
          fileReference: Buffer.from(
            atob(l.file_reference_base64.replace(/-/g, '+').replace(/_/g, '/'))
          ),
          size: BigInt(0),
        } as GramJs.Document);
...
function getLimit(
  ...
  key: Limit,
  fallbackKey: ApiLimitType
) {
  const defaultLimit =
    appConfig[`${key}_default`] || DEFAULT_LIMITS[fallbackKey][0];
  const premiumLimit =
    appConfig[`${key}_premium`] || DEFAULT_LIMITS[fallbackKey][1];
  return [defaultLimit, premiumLimit] as const;
}

export function buildAppConfig(
  json: GramJs.JsonArray,
  hash: number
): ApiAppConfig {
  ...
  return {
    ...
    defaultEmojiStatusesStickerSetId:
      appConfig.default_emoji_statuses_stickerset_id,
    ...
    hash,
    limits: {
      uploadMaxFileparts: getLimit(
        appConfig,
        'upload_max_fileparts',
        'uploadMaxFileparts'
      ),
      stickersFaved: getLimit(
        appConfig,
        'stickers_faved_limit',
        'stickersFaved'
      ),
      savedGifs: getLimit(appConfig, 'saved_gifs_limit', 'savedGifs'),
      dialogFiltersChats: getLimit(
        appConfig,
        'dialog_filters_chats_limit',
        'dialogFiltersChats'
      ),
      dialogFilters: getLimit(
        appConfig,
        'dialog_filters_limit',
        'dialogFilters'
      ),
      dialogFolderPinned: getLimit(
        appConfig,
        'dialogs_pinned_limit',
        'dialogFolderPinned'
      ),
      captionLength: getLimit(
        appConfig,
        'caption_length_limit',
        'captionLength'
      ),
      channels: getLimit(appConfig, 'channels_limit', 'channels'),
      channelsPublic: getLimit(
        appConfig,
        'channels_public_limit',
        'channelsPublic'
      ),
