...
import { formatTimeDuration } from '../../../util/dateFormat';
...
export function buildApiSponsoredMessage(
  mtpMessage: GramJs.SponsoredMessage
  ...
  const chatInviteTitle = chatInvite
    ? chatInvite instanceof GramJs.ChatInvite
      ? chatInvite.title
      : !(chatInvite.chat instanceof GramJs.ChatEmpty)
...
export function buildApiMessageFromShort(
  ...
  return buildApiMessageWithChatId(chatId, {
    ...mtpMessage,
    fromId: buildPeer(
      mtpMessage.out ? currentUserId : buildApiPeerId(mtpMessage.userId, 'user')
    ),
  });
}
...
export function buildApiMessageWithChatId(
  ...
  const fromId = mtpMessage.fromId
    ? getApiChatIdFromMtpPeer(mtpMessage.fromId)
    : undefined;
  const peerId = mtpMessage.peerId
    ? getApiChatIdFromMtpPeer(mtpMessage.peerId)
    : undefined;
  const isChatWithSelf = !fromId && chatId === currentUserId;
  const isOutgoing = mtpMessage.out || (isChatWithSelf && !mtpMessage.fwdFrom);
  ...
  const isInvoiceMedia =
    mtpMessage.media instanceof GramJs.MessageMediaInvoice &&
    Boolean(mtpMessage.media.extendedMedia);

  const { replyToMsgId, replyToTopId, replyToPeerId } =
    mtpMessage.replyTo || {};
  ...
  const forwardInfo =
    mtpMessage.fwdFrom &&
    buildApiMessageForwardInfo(mtpMessage.fwdFrom, isChatWithSelf);
  ...
  const isInAlbum =
    Boolean(groupedId) &&
    !(content.document || content.audio || content.sticker);
  const shouldHideKeyboardButtons =
    mtpMessage.replyMarkup instanceof GramJs.ReplyKeyboardHide;
  const isHideKeyboardSelective =
    mtpMessage.replyMarkup instanceof GramJs.ReplyKeyboardHide &&
  ...
  return {
    ...
    senderId:
      fromId || (mtpMessage.out && mtpMessage.post && currentUserId) || chatId,
    ...
    reactions:
      mtpMessage.reactions && buildMessageReactions(mtpMessage.reactions),
    emojiOnlyCount,
    replyToMsgId,
    //...(forumTopic && { isTopicReply: true }),
    ...(replyToMsgId && { replyToMessageId: replyToMsgId }),
    ...(replyToPeerId && {
      replyToChatId: getApiChatIdFromMtpPeer(replyToPeerId),
    }),
    replyToTopId,
    ...
    ...(mtpMessage.viaBotId && {
      viaBotId: buildApiPeerId(mtpMessage.viaBotId, 'user'),
    }),
    ...(replies && {
      repliesThreadInfo: buildThreadInfo(replies, mtpMessage.id, chatId),
    }),
    ...(postAuthor && { postAuthorTitle: postAuthor }),
    isProtected,
    isForwardingAllowed,
    isLiked: mtpMessage.isLiked,
    likes: mtpMessage.likes,
  };
}

export function buildMessageReactions(
  reactions: GramJs.MessageReactions
): ApiReactions {
  const { recentReactions, results, canSeeList } = reactions;

  return {
    canSeeList,
    results: results
      .map(buildReactionCount)
      .filter(Boolean)
      .sort(reactionCountComparator),
    // recentReactions: recentReactions
    //   ?.map(buildMessagePeerReaction)
    //   .filter(Boolean),
  };
}
...
function buildReactionCount(
  reactionCount: GramJs.ReactionCount
): ApiReactionCount | undefined {
...
export function buildMessagePeerReaction(
  userReaction: GramJs.MessagePeerReaction
): ApiUserReaction | undefined {
  const { peerId, reaction, big, unread } = userReaction;
...
export function buildApiReaction(
  reaction: GramJs.TypeReaction
...
export function buildApiAvailableReaction(
  availableReaction: GramJs.AvailableReaction
): ApiAvailableReaction {
  const {
    selectAnimation,
    staticIcon,
  ...
  return {
    ...
    aroundAnimation: aroundAnimation
      ? buildApiDocument(aroundAnimation)
      : undefined,
...
export function buildMessageContent(
  ...
  const hasUnsupportedMedia =
    mtpMessage.media instanceof GramJs.MessageMediaUnsupported;
...
export function buildMessageDraft(draft: GramJs.TypeDraftMessage) {
  ...
  const { message, entities, replyToMsgId, date } = draft;

  return {
    formattedText: message
      ? buildMessageTextContent(message, entities)
      : undefined,
...
export function buildMessageMediaContent(
  media: GramJs.TypeMessageMedia
): ApiMessage['content'] | undefined {
  if ('ttlSeconds' in media && media.ttlSeconds) {
    return undefined;
  }

  if (
    'extendedMedia' in media &&
    media.extendedMedia instanceof GramJs.MessageExtendedMedia
  ) {
...
function buildApiMessageForwardInfo(
  ...
  const savedFromPeerId =
    fwdFrom.savedFromPeer && getApiChatIdFromMtpPeer(fwdFrom.savedFromPeer);
  ...
  return {
    ...
    isLinkedChannelPost: Boolean(
      fwdFrom.channelPost && savedFromPeerId && !isChatWithSelf
    ),
...
function buildPhoto(media: GramJs.TypeMessageMedia): ApiPhoto | undefined {
  if (
    ...
    !media.photo ||
    !(media.photo instanceof GramJs.Photo)
  ) {
    return undefined;
  }
...
export function buildVideoFromDocument(
  ...
  const { id, mimeType, thumbs, size, attributes } = document;
  ...
  const videoAttr = attributes.find(
    (a: any): a is GramJs.DocumentAttributeVideo =>
      a instanceof GramJs.DocumentAttributeVideo
  );
  ...
  const gifAttr = attributes.find(
    (a: any): a is GramJs.DocumentAttributeAnimated =>
      a instanceof GramJs.DocumentAttributeAnimated
  );
...
function buildAudio(media: GramJs.TypeMessageMedia): ApiAudio | undefined {
  ...
  const audioAttribute = media.document.attributes.find(
    (attr: any): attr is GramJs.DocumentAttributeAudio =>
  ...
  const thumbnailSizes =
    ...
    media.document.thumbs
      .filter(
        (thumb): thumb is GramJs.PhotoSize => thumb instanceof GramJs.PhotoSize
      )
...
export function buildApiDocument(
  ...
  const photoSize =
    thumbs &&
    thumbs.find(
      (s: any): s is GramJs.PhotoSize => s instanceof GramJs.PhotoSize
    );
  if (photoSize) {
    mediaSize = {
      width: photoSize.w,
    ...
    if (SUPPORTED_IMAGE_CONTENT_TYPES.has(mimeType)) {
      ...
      const imageAttribute = attributes.find(
        (a: any): a is GramJs.DocumentAttributeImageSize =>
...
function buildInvoiceFromMedia(
  media: GramJs.TypeMessageMedia
): ApiInvoice | undefined {
...
function buildLocationFromMedia(
  media: GramJs.TypeMessageMedia
...
function buildGeoLive(
  media: GramJs.MessageMediaGeoLive
): ApiLocation | undefined {
...
function buildGeoPoint(
  geo: GramJs.TypeGeoPoint
): ApiLocation['geo'] | undefined {
...
function buildGame(media: GramJs.MessageMediaGame): ApiGame | undefined {
  ...
  const photo =
    apiPhoto instanceof GramJs.Photo ? buildApiPhoto(apiPhoto) : undefined;
  const document =
    apiDocument instanceof GramJs.Document
      ? buildApiDocument(apiDocument)
      : undefined;
...
export function buildInvoice(media: GramJs.MessageMediaInvoice): ApiInvoice {
  ...
  const preview =
    extendedMedia instanceof GramJs.MessageExtendedMediaPreview
      ? buildApiMessageExtendedMediaPreview(extendedMedia)
      : undefined;
...
export function buildPollResults(
  ...
  const results =
    rawResults &&
    rawResults.map(({ option, chosen, correct, voters }) => ({
      isChosen: chosen,
      isCorrect: correct,
...
export function buildWebPage(
  ...
  let video;
  if (
    document instanceof GramJs.Document &&
...
function buildAction(
  ...
  if (action instanceof GramJs.MessageActionChatCreate) {
    ...
    if (!senderId || targetUserIds.includes(senderId)) {
      ...
      text = senderId === currentUserId ? 'youInvited' : 'invited';
    }
    ...
    if (!senderId || targetUserIds.includes(senderId)) {
      text = 'Notification.YouJoinedGroupByLink';
    } else {
      text = 'joinedByLink';
    }
    ...
    if (action.duration) {
      ...
      translationValues.push(`${mins} min${mins > 1 ? 's' : 's'}`);
    } else {
    ...
    //   const isVideo =
    //     action.photo instanceof GramJs.Photo && action.photo.videoSizes?.length;
    //   text =
    //     senderId === currentUserId
    //       ? isVideo
...
function buildReplyButtons(
  message: UniversalMessage,
  shouldSkipBuyButton?: boolean
  ...
  if (!replyMarkup) {
    if (
      ...
      if (media.webpage.type === 'Ello_message') {
        return {
          inlineButtons: [
            [
              {
  ...
  if (
    !(
      ...
      replyMarkup instanceof GramJs.ReplyInlineMarkup
    )
  ) {
    return undefined;
  }
  ...
  const markup = replyMarkup.rows.map(({ buttons }) => {
    return buttons
      .map((button): ApiKeyboardButton | undefined => {
        const { text } = button;
        ...
        if (button instanceof GramJs.KeyboardButtonBuy) {
          if (
            media instanceof GramJs.MessageMediaInvoice &&
            media.receiptMsgId
          ) {
            return {
              type: 'receipt',
...
function getFilenameFromDocument(
  ...
  const filenameAttribute = attributes.find(
    (a: any): a is GramJs.DocumentAttributeFilename =>
      a instanceof GramJs.DocumentAttributeFilename
  );
...
export function buildLocalMessage(
  ...
  const message = {
    ...
    ...(replyingToTopId && { replyToTopId: replyingToTopId }),
    ...(groupedId && {
      groupedId,
...
export function buildLocalForwardedMessage({
  ...
  const asIncomingInChatWithSelf =
    toChat.id === currentUserId &&
    (fromChatId !== toChat.id || message.forwardInfo) &&
    !isAudio;
  const shouldHideText =
    Object.keys(content).length > 1 && content.text && noCaptions;
  const shouldDropCustomEmoji = !isCurrentUserPremium;
  const strippedText =
    content.text?.entities && shouldDropCustomEmoji
      ? {
          text: content.text.text,
          entities: content.text.entities.filter(
  ...
  return {
    ...
    ...(message.chatId === currentUserId &&
      !noAuthors && { forwardInfo: message.forwardInfo }),
    ...(scheduledAt && { isScheduled: true }),
  };
}
      ...
      if (SUPPORTED_VIDEO_CONTENT_TYPES.has(mimeType)) {
        ...
        return {
          video: {
            ...
            blobUrl,
            ...(previewBlobUrl && {
              thumbnail: { width, height, dataUri: previewBlobUrl },
            }),
            size,
            isSpoiler: shouldSendAsSpoiler,
          },
...
export function buildApiWebDocument(
  document?: GramJs.TypeWebDocument
): ApiWebDocument | undefined {
  if (!document) return undefined;

  const { url, size, mimeType } = document;
  const accessHash =
    document instanceof GramJs.WebDocument
      ? document.accessHash.toString()
      : undefined;
  const sizeAttr = document.attributes.find(
    (attr): attr is GramJs.DocumentAttributeImageSize =>
...
export function buildApiMessageEntity(
  entity: GramJs.TypeMessageEntity
): ApiMessageEntity {
...
function buildThreadInfo(
  ...
  const { channelId, replies, maxId, readMaxId, recentRepliers, comments } =
    messageReplies;

  const apiChannelId = channelId
    ? buildApiPeerId(channelId, 'channel')
    : undefined;
...
// export function buildApiFormattedText(
//   textWithEntities: GramJs.TextWithEntities
// ): ApiFormattedText {
