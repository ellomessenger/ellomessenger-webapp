...
export function getMessageOriginalId(message: ApiMessage) {
  return (
    ...
    `${message.id} * ${message.chatId.replace('-', '')}`
  );
}
...
export function isReplyMessage(message: ApiMessage) {
  return Boolean(message.replyToMsgId);
}
...
export function isMessageLink(message: ApiMessage) {
  const { text } = message.content;
  if (text?.text.includes('http') && !text.text.includes('\n')) return true;
}
