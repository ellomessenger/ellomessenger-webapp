...
function unsafeMigrateCache(cached: GlobalState, initialState: GlobalState) {
  ...
  if ('canAutoPlayVideos' in cached.settings.byKey) {
    cached.settings.performance.autoplayVideos =
      cached.settings.byKey.canAutoPlayVideos;
    delete cached.settings.byKey.canAutoPlayVideos;
  }

  if ('canAutoPlayGifs' in cached.settings.byKey) {
    cached.settings.performance.autoplayGifs =
      cached.settings.byKey.canAutoPlayGifs;
  ...
  if (!cached.users.fullInfoById) {
    const result = Object.entries(cached.users.byId).reduce(
      (acc, [id, user]) => {
        if ('fullInfo' in user) {
  ...
  if (!cached.chats.fullInfoById) {
    const result = Object.entries(cached.chats.byId).reduce(
      (acc, [id, chat]) => {
        if ('fullInfo' in chat) {
...
export function serializeGlobal<T extends GlobalState>(global: T) {
  const reducedGlobal: GlobalState = {
    ...
    //feedFilter: reduceFeedFilter(global),
...
function reduceMessages<T extends GlobalState>(
  ...
  const unreadFeedMessage: GlobalState['messages']['unreadFeedMessage'] = [];
  const currentChatIds = compact(
    Object.values(global.byTabId).map(({ id: tabId }) =>
      selectCurrentMessageList(global, tabId)
    )
  ...
  chatIdsToSave.forEach((chatId) => {
    ...
    const threadIdsToSave = threadIds.length
      ? [MAIN_THREAD_ID, ...threadIds]
      : [MAIN_THREAD_ID];
...
function reduceFeedFilter<T extends GlobalState>(
  global: T
): GlobalState['feedFilter'] {
  const { scrollOffset, customBg, all } = global.feedFilter || {};

  return { scrollOffset, customBg };
}
