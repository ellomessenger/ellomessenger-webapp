...
import { buildPeerId, isChatAdult, isPaidChannel } from '../helpers';

export function replaceChatListIds<T extends GlobalState>(
  global: T,
  type: 'active' | 'archived' | 'feed' | 'bots',
...
export function updateFeedChatListIds<T extends GlobalState>(
  global: T,
  newState: T['feedFilter']
) {
  const { all, hidden, pinned, show_adult, show_only_subs } = newState || {};
  const allArr = buildPeerId(all);
  const hiddenArr = buildPeerId(hidden);
  const pinnedArr = buildPeerId(pinned);
  const newIds = allArr
    ?.filter((id) => {
      const chat = global.chats.byId[id];
      if (
        (hiddenArr && hiddenArr.includes(id)) ||
        (!show_adult && isChatAdult(chat)) ||
        (show_only_subs && !isPaidChannel(chat))
      ) {
        return false;
      }
      return id;
    })
    .sort((a) => (pinnedArr?.includes(a) ? -1 : 1));

  return replaceChatListIds(global, 'feed', newIds);
}

export function replaceChats<T extends GlobalState>(
  global: T,
  newById: Record<string, ApiChat>
): T {
  return {
    ...global,
    chats: {
...
export function updateChat<T extends GlobalState>(
  global: T,
  chatId: string,
  chatUpdate: Partial<ApiChat>,
  photo?: ApiPhoto,
  noOmitUnreadReactionCount = false
): T {
  const { byId } = global.chats;

  const updatedChat = getUpdatedChat(
    global,
    chatId,
    chatUpdate,
    photo,
    noOmitUnreadReactionCount
  );
  if (!updatedChat) {
    return global;
  }
...
export function updateChats<T extends GlobalState>(
  global: T,
  newById: Record<string, ApiChat>
): T {
  const updatedById = Object.keys(newById).reduce(
    (acc: Record<string, ApiChat>, id) => {
...
export function addChats<T extends GlobalState>(
  global: T,
  newById: Record<string, ApiChat>
): T {
  const { byId } = global.chats;
  ...
  const addedById = Object.keys(newById).reduce<Record<string, ApiChat>>(
    (acc, id) => {
...
export function updateChatListType<T extends GlobalState>(
  ...
  (Object.keys(currentListIds) as Array<keyof typeof currentListIds>).forEach(
    (listTypeKey) => {
      ...
      if (listTypeKey === listType && !currentFolderList.includes(chatId)) {
        ...
        currentFolderList.includes(chatId)
      ) {
        currentListIds = {
          ...currentListIds,
...
export function removeChatFromChatLists<T extends GlobalState>(
  ...
  Object.entries(lists).forEach(([listType, listIds]) => {
    if (listIds) {
      global = replaceChatListIds(
        global,
        listType as keyof typeof lists,
        listIds.filter((id) => id !== chatId)
      );
    }
  });
...
export function leaveChat<T extends GlobalState>(
  global: T,
  leftChatId: string
): T {
  ...
  if (listIds) {
    global = replaceChatListIds(
      global,
      listType,
      listIds.filter((listId) => listId !== leftChatId)
    );
    global = replaceChatListIds(
      global,
      'feed',
...
export function addChatMembers<T extends GlobalState>(
  global: T,
  chat: ApiChat,
  membersToAdd: ApiChatMember[]
): T {
