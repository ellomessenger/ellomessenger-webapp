...
import { isActionMessage, isLocalMessageId } from '../helpers';
...
export function updateTabThread<T extends GlobalState>(
  ...
  chatId: string,
  threadId: ThreadId,
  threadUpdate: Partial<TabThread>,
  ...[tabId = getCurrentTabId()]: TabArgs<T>
): T {
...
export function updateThread<T extends GlobalState>(
  ...
  chatId: string,
  threadId: ThreadId,
  threadUpdate: Partial<Thread>
): T {
  const current = global.messages.byChatId[chatId];

  if (threadUpdate.listedIds?.length) {
    const lastListedId =
      threadUpdate.listedIds[threadUpdate.listedIds.length - 1];
    if (lastListedId) {
...
function updateMessageStore<T extends GlobalState>(
  global: T,
  chatId: string,
  update: Partial<MessageStoreSections>
): T {
  const current = global.messages.byChatId[chatId] || {
    byId: {},
    threadsById: {},
...
export function replaceTabThreadParam<
...
>(
  ...
  threadId: ThreadId,
  paramName: K,
  newValue: TabThread[K] | undefined,
  ...[tabId = getCurrentTabId()]: TabArgs<T>
) {
...
export function addMessages<T extends GlobalState>(
  ...
  const addedByChatId = messages.reduce(
    (messagesByChatId, message: ApiMessage) => {
...
export function addChatMessagesById<T extends GlobalState>(
  ...
  if (
    byId &&
    Object.keys(newById).every((newId) => Boolean(byId[Number(newId)]))
  ) {
    return global;
  }
...
export function addFeedMessagesById<T extends GlobalState>(
  global: T,
  messages: ApiMessage[],
  page?: number
) {
  const byId = selectFeedMessages(global);

  if (
    byId &&
    messages.every(({ id, chatId }) => Boolean(byId[`${id}${chatId}`]))
  ) {
    return global;
  }

  const addedById = messages.reduce((messagesById, message: ApiMessage) => {
    messagesById[`${message.id}${message.chatId}`] = message;
  ...
  return {
    ...
    messages: {
      ...global.messages,
      inFeed: {
        ...global.messages.inFeed,
        page: page || ++global.messages.inFeed.page!,
        byId: {
          ...addedById,
          ...byId,
        },
...
export function addFeedMessage<T extends GlobalState>(
  global: T,
  message: ApiMessage
) {
  const byId = selectFeedMessages(global);

  if (!message || !global.chats.listIds.feed?.includes(message.chatId!)) {
    return global;
  }

  if (byId) {
    byId[`${message.id}${message.chatId}`] = message;
  }
...
export function addFeedExploreById<T extends GlobalState>(
  global: T,
  messages: ApiMessage[],
  page?: number
) {
  const byId = selectFeedExplore(global);
  ...
  return {
    ...
    messages: {
      ...global.messages,
      inExplore: {
        ...global.messages.inExplore,
        page: page || ++global.messages.inExplore.page,
        byId: {
          ...addedById,
          ...byId,
        },
...
export function updateFeedMessage<T extends GlobalState>(
  global: T,
  chatId: string,
  messageId: number,
  ...
  const byId = selectFeedMessages(global) || {};
  const key = `${messageId}${chatId}`;
  const message = byId[key];
...
export function deleteChatMessages<T extends GlobalState>(
  ...
  const deletedForwardedPosts = Object.values(
    pickTruthy(byId, messageIds)
  ).filter(({ forwardInfo }) => forwardInfo?.isLinkedChannelPost);
  ...
  threadIds.forEach((threadId) => {
    ...
    messageIds.forEach((messageId) => {
      if (listedIds?.includes(messageId)) {
        ...
        if (newMessageCount !== undefined && !isLocalMessageId(messageId))
          newMessageCount -= 1;
      }
  ...
  if (deletedForwardedPosts.length) {
    Object.values(global.byTabId).forEach(({ id: tabId }) => {
      ...
      deletedForwardedPosts.forEach((message) => {
        ...
        const originalPost = selectChatMessage(
          global,
          fromChatId!,
          fromMessageId!
        );
        ...
        if (originalPost) {
          global = updateChatMessage(global, fromChatId!, fromMessageId!, {
            repliesThreadInfo: undefined,
          });
        }
      });
    });
...
export function deleteChatScheduledMessages<T extends GlobalState>(
  ...
  if (scheduledIds) {
    ...
    Object.entries(global.messages.byChatId[chatId].threadsById).forEach(
      ([threadId, thread]) => {
        if (thread.scheduledIds) {
          const newScheduledIds = thread.scheduledIds.filter(
...
export function updateListedIds<T extends GlobalState>(
  ...
  return replaceThreadParam(
    ...
    threadId,
    'listedIds',
    orderHistoryIds([...(listedIds || []), ...newIds])
  );
}
...
export function updateThreadInfo<T extends GlobalState>(
  ...
  chatId: string,
  threadId: ThreadId,
  update: Partial<ApiThreadInfo> | undefined,
  ...
  if (!doNotUpdateLinked) {
    const linkedUpdate = pick(newThreadInfo, [
      'messagesCount',
      'lastMessageId',
      'lastReadInboxMessageId',
    ]);
    if (newThreadInfo.isComments) {
      if (newThreadInfo.threadId) {
        global = updateThreadInfo(
          global,
          newThreadInfo.chatId,
          newThreadInfo.threadId,
          linkedUpdate,
          true
        );
      }
    } else if (newThreadInfo.originChannelId && newThreadInfo.topMessageId) {
      global = updateThreadInfo(
        global,
        newThreadInfo.originChannelId,
        newThreadInfo.topMessageId,
        linkedUpdate,
        true
      );
    }
...
function updateScheduledMessages<T extends GlobalState>(
  global: T,
  chatId: string,
  update: Partial<{ byId: Record<number, ApiMessage> }>
): T {
...
export function addActiveMessageMediaDownload<T extends GlobalState>(
  ...
  const currentIds =
    (message.isScheduled ? byChatId?.scheduledIds : byChatId?.ids) || [];
    ...
    global,
    {
      activeDownloads: {
        byChatId: {
          ...
          [message.chatId]: {
            ...byChatId,
            [message.isScheduled ? 'scheduledIds' : 'ids']: unique([
              ...currentIds,
              message.id,
            ]),
          },
    ...
    global,
    {
      activeDownloads: {
        byChatId: {
          ...
          [message.chatId]: {
            ...byChatId,
            [message.isScheduled ? 'scheduledIds' : 'ids']: currentIds.filter(
              (id) => id !== message.id
            ),
          },
        },
...
export function toggleMessageSelection<T extends GlobalState>(
  ...
  const { selectedMessages: oldSelectedMessages } = selectTabState(
    global,
    tabId
  );
  if (groupedId) {
  ...
  let newMessageIds;
  const newSelectedMessageIds = selectedMessageIds.filter(
    (id) => !messageIds.includes(id)
  );
  if (newSelectedMessageIds && !newSelectedMessageIds.length) {
    ...
    const viewportIds = selectCurrentMessageIds(
      global,
      chatId,
      threadId,
      messageListType,
  ...
  if (channelPostId && fromChatId) {
    ...
    if (threadInfoOld) {
      global = replaceThreadParam(global, chatId, channelPostId, 'threadInfo', {
        ...
        lastMessageId,
        messagesCount:
          (threadInfoOld.messagesCount || 0) + (isDeleting ? -1 : 1),
      });
    }
  }
  return global;
...
export function updateFeedUnreadMessages<T extends GlobalState>(
  global: T,
  ...
  const prevUnread = global.messages.unreadFeedMessage;

  const unreadFeedMessage = orderBy([...prevUnread, message], ['date'], 'asc');
