import type { GlobalState, TabState, TabArgs, IBankRequisites } from '../types';
...
export function updatePayment<T extends GlobalState>(
  global: T,
  update: Partial<TabState['payment']>,
...
export function updateBanksRequisites<T extends GlobalState>(
  global: T,
  update: IBankRequisites
) {
  return [global.bankWithdrawsRequisites, update];
}
...
export function setStripeCardInfo<T extends GlobalState>(
  global: T,
  cardInfo: { type: string; id: string },
  ...[tabId = getCurrentTabId()]: TabArgs<T>
): T {
...
export function setSmartGlocalCardInfo<T extends GlobalState>(
  ...
  return updatePayment(
    global,
    { smartGlocalCredentials: { ...cardInfo } },
    tabId
  );
}
...
export function setReceipt<T extends GlobalState>(
  ...
  const { photo, text, title } = messageInvoice || {};
...
export function closeInvoice<T extends GlobalState>(
  ...
  return updatePayment(
    global,
    { isPaymentModalOpen: undefined, isExtendedMedia: undefined },
    tabId
  );
}
