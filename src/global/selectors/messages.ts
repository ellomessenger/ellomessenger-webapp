...
export function selectChatMessages<T extends GlobalState>(
  global: T,
  chatId: string
) {
...
export function selectChatScheduledMessages<T extends GlobalState>(
  global: T,
  chatId: string
) {
...
export function selectTabThreadParam<
  T extends GlobalState,
  K extends keyof TabThread
>(
  global: T,
  chatId: string,
  threadId: ThreadId,
...
export function selectThreadParam<
  T extends GlobalState,
  K extends keyof Thread
>(global: T, chatId: string, threadId: ThreadId, key: K) {
...
export function selectListedIds<T extends GlobalState>(
  global: T,
  chatId: string,
  threadId: ThreadId
) {
...
export function selectOutlyingListByMessageId<T extends GlobalState>(
  ...
  return outlyingLists.find((list: string | any[]) => {
...
export function selectFeedMessages<T extends GlobalState>(global: T) {
  return global.messages.inFeed.byId;
}

export function selectFeedExplore<T extends GlobalState>(global: T) {
  return global.messages.inExplore?.byId;
}
...
export function selectPinnedIds<T extends GlobalState>(
  global: T,
  chatId: string,
  threadId: ThreadId
) {
...
export function selectScheduledIds<T extends GlobalState>(
  global: T,
  chatId: string,
  threadId: ThreadId
) {
...
export function selectScrollFeedOffset<T extends GlobalState>(
  global: T,
  ...[tabId = getCurrentTabId()]: TabArgs<T>
) {
  return global.feedFilter?.scrollOffset;
}

export function selectLastScrollOffset<T extends GlobalState>(
  global: T,
  chatId: string,
  threadId: ThreadId
) {
...
export function selectReplyingToId<T extends GlobalState>(
  global: T,
  chatId: string,
  threadId: ThreadId
) {
...
export function selectEditingId<T extends GlobalState>(
  global: T,
  chatId: string,
  threadId: ThreadId
) {
...
export function selectEditingDraft<T extends GlobalState>(
  global: T,
  chatId: string,
  threadId: ThreadId
) {
...
export function selectEditingScheduledId<T extends GlobalState>(
  global: T,
  chatId: string
) {
  return selectThreadParam(
    global,
    chatId,
    MAIN_THREAD_ID,
    'editingScheduledId'
...
export function selectEditingScheduledDraft<T extends GlobalState>(
  global: T,
  chatId: string
) {
  return selectThreadParam(
    ...
    chatId,
    MAIN_THREAD_ID,
    'editingScheduledDraft'
  );
}

export function selectDraft<T extends GlobalState>(
  global: T,
  chatId: string,
  threadId: ThreadId
) {
...
export function selectNoWebPage<T extends GlobalState>(
  global: T,
  chatId: string,
  threadId: ThreadId
) {
...
export function selectThreadInfo<T extends GlobalState>(
  global: T,
  chatId: string,
  threadId: ThreadId
) {
...
export function selectRepliesThreadInfo<T extends GlobalState>(
  global: T,
  chatId: string,
  threadId: ThreadId
) {
  const replyMessage = selectChatMessage(global, chatId, Number(threadId));
  return replyMessage?.repliesThreadInfo;
}

export function selectThreadTopMessageId<T extends GlobalState>(
  global: T,
  chatId: string,
  threadId: ThreadId
) {
  ...
  return threadInfo.topMessageId || threadInfo.threadId;
}

export function selectFirstMessageId<T extends GlobalState>(
  global: T,
  chatId: string,
  threadId: ThreadId
) {
...
export function selectThreadOriginChat<T extends GlobalState>(
  global: T,
  chatId: string,
  threadId: ThreadId
) {
...
export function selectThreadMessagesCount(
  global: GlobalState,
  chatId: string,
  threadId: ThreadId
) {
...
export function selectThreadByMessage<T extends GlobalState>(
  global: T,
  message: ApiMessage
) {
...
export function selectIsViewportFeedNewest<T extends GlobalState>(global: T) {
  const idx = global.chats.listIds.feed?.findIndex((id) =>
    selectIsViewportNewest(global, id, MAIN_THREAD_ID, getCurrentTabId())
  );
  if (!idx || idx === -1) {
    return true;
  }
...
export function selectIsViewportNewest<T extends GlobalState>(
  ...
  if (
    isLocalMessageId(lastMessageId) &&
    !selectChatMessage(global, chatId, lastMessageId)
  ) {
    return true;
  }
...
export function selectChatMessage<T extends GlobalState>(
  global: T,
  chatId: string,
  messageId: number
) {
...
export function selectScheduledMessage<T extends GlobalState>(
  global: T,
  chatId: string,
  messageId: number
) {
...
export function selectChatMessageByPollId<T extends GlobalState>(
  global: T,
  pollId: string
) {
...
export function selectFocusedMessageId<T extends GlobalState>(
  ...
  const { chatId: focusedChatId, messageId } =
...
export function selectIsMessageFocused<T extends GlobalState>(
  ...
  return focusedId
    ? focusedId === message.id || focusedId === message.previousLocalId
    : false;
}
...
export function selectIsMessageUnread<T extends GlobalState>(
  ...
  return (
    ...
    lastReadOutboxMessageId < message.id
  );
}
...
export function selectSender<T extends GlobalState>(
  global: T,
  message: ApiMessage
): ApiPeer | undefined {
  const { senderId, forwardInfo } = message;
  if (!senderId && !forwardInfo) {
    return undefined;
  }
  ...
  if (forwardInfo?.fromChatId)
    return selectPeer(global, forwardInfo.fromChatId);
  if (senderId) return selectPeer(global, senderId);
}
...
export function selectReplySender<T extends GlobalState>(
  ...
  if (isForwarded) {
    ...
    if (senderUserId) {
      return isUserId(senderUserId)
        ? selectUser(global, senderUserId)
        : selectChat(global, senderUserId);
    }
  ...
  return isUserId(senderId)
    ? selectUser(global, senderId)
    : selectChat(global, senderId);
}
...
export function selectForwardedSender<T extends GlobalState>(
  ...
  if (forwardInfo.isChannelPost && forwardInfo.fromChatId) {
    ...
    return (
      selectUser(global, forwardInfo.senderUserId) ||
      selectChat(global, forwardInfo.senderUserId)
    );
  }
...
export function selectCanDeleteOwnerTopic<T extends GlobalState>(
  global: T,
  chatId: string,
  topicId: number
) {
...
export function selectCanDeleteTopic<T extends GlobalState>(
  global: T,
  chatId: string,
  topicId: number
) {
  ...
  return (
    ...
    (chat.isForum && selectCanDeleteOwnerTopic(global, chat.id, topicId))
  );
}

export function selectThreadIdFromMessage<T extends GlobalState>(
  global: T,
  message: ApiMessage
): number {
  const chat = selectChat(global, message.chatId);
  const { content } = message;
  const { replyToMsgId, replyToTopId, isForumTopic } = message || {};
...
export function selectTopicFromMessage<T extends GlobalState>(
  global: T,
  message: ApiMessage
) {
...
export function selectAllowedMessageActions<T extends GlobalState>(
  global: T,
  message: ApiMessage,
  threadId: ThreadId
) {
  ...
  const canReply =
    ...
    (!messageTopic ||
      !messageTopic.isClosed ||
      messageTopic.isOwner ||
  ...
  let canPin =
    !isLocal && !isServiceNotification && !isAction && hasPinPermission;
  let canUnpin = false;
  ...
  const canEdit =
    ...
    (isOwn ||
      (isChannel &&
        (chat.isCreator || getHasAdminRight(chat, 'editMessages'))));
  ...
  const hasFavoriteSticker =
    hasSticker && selectIsStickerFavorite(global, message.content.sticker!);
  ...
  const canDownload = Boolean(
    content.webPage?.document ||
      content.webPage?.video ||
      content.webPage?.photo ||
      content.audio ||
      content.voice ||
      content.photo ||
      content.video ||
  ...
  const canRevote =
    !poll?.summary.closed &&
    !poll?.summary.quiz &&
    poll?.results.results?.some((r) => r.isChosen);
...
export function selectCanDeleteSelectedMessages<T extends GlobalState>(
  ...
  const { messageIds: selectedMessageIds } =
    selectTabState(global, tabId).selectedMessages || {};
  ...
  const messageActions = selectedMessageIds
    .map(
      (id) =>
        chatMessages[id] &&
        selectAllowedMessageActions(global, chatMessages[id], threadId)
    )
    .filter(Boolean);
...
export function selectIsDownloading<T extends GlobalState>(
  ...
  const activeInChat = selectTabState(global, tabId).activeDownloads.byChatId[
    message.chatId
  ];
  ...
  return Boolean(
    message.isScheduled
      ? activeInChat.scheduledIds?.includes(message.id)
      : activeInChat.ids?.includes(message.id)
  );
}
...
export function selectUploadProgress<T extends GlobalState>(
  global: T,
  message: ApiMessage
) {
  const messageKey = message.previousLocalId || message.id;
  return global.fileUploads.byMessageLocalId[messageKey]?.progress;
}

export function selectRealLastReadId<T extends GlobalState>(
  global: T,
  chatId: string,
  threadId: ThreadId
) {
  if (threadId === MAIN_THREAD_ID) {
    ...
    return Math.min(
      ...
      threadInfo.lastMessageId || Infinity
    );
  }
}
...
export function selectFirstUnreadId<T extends GlobalState>(
  ...
  if (threadId === MAIN_THREAD_ID) {
    ...
    if (
      !threadInfo ||
      (threadInfo.lastMessageId !== undefined &&
        threadInfo.lastMessageId === threadInfo.lastReadInboxMessageId)
    ) {
      return undefined;
    }
  }
  ...
  const lastReadServiceNotificationId =
    chatId === SERVICE_NOTIFICATIONS_USER_ID
      ? global.serviceNotifications.reduce((max, notification) => {
          return !notification.isUnread && notification.id > max
            ? notification.id
            : max;
        }, -1)
      : -1;
  ...
  function findAfterLastReadId(listIds: number[]) {
    return listIds.find((id) => {
      return (
        ...
        (!byId[id].isOutgoing ||
          byId[id].content.action?.isTopicAction ||
          byId[id].isFromScheduled) &&
  ...
  if (outlyingLists?.length) {
    const found = outlyingLists
      .map((list: number[]) => findAfterLastReadId(list))
      .filter(Boolean)[0];
    if (found) {
      return found;
    }
...
export function selectCommonBoxChatId<T extends GlobalState>(
  global: T,
  messageId: number
) {
  const fromLastMessage = Object.values(global.chats.byId).find(
    (chat) =>
      isCommonBoxChat(chat) &&
      chat.lastMessage &&
...
export function selectMessageIdsByGroupId<T extends GlobalState>(
  global: T,
  chatId: string,
  groupedId: string
) {
...
>(
  ...
  const messageId = findLast(viewportIds, (id: number) =>
    selectShouldDisplayReplyKeyboard(global, chatMessages[id])
  );

  const replyHideMessageId = findLast(viewportIds, (id: number) =>
    selectShouldHideReplyKeyboard(global, chatMessages[id])
  );
...
function selectShouldHideReplyKeyboard<T extends GlobalState>(
  global: T,
  message: ApiMessage
) {
  ...
  if (isHideKeyboardSelective) {
    ...
    const replyMessage = selectChatMessage(
      global,
      message.chatId,
      replyToMsgId
    );
...
function selectShouldDisplayReplyKeyboard<T extends GlobalState>(
  global: T,
  message: ApiMessage
) {
  if (!message) return false;
...
export function selectCanAutoLoadMedia<T extends GlobalState>(
  ...
  const isPhoto = Boolean(
    getMessagePhoto(message) || getMessageWebPagePhoto(message)
  );
  const isVideo = Boolean(
    getMessageVideo(message) || getMessageWebPageVideo(message)
  );
...
function canAutoLoadMedia<T extends GlobalState>({
  ...
  const isMediaFromContact = Boolean(
    sender &&
...
export function selectLastServiceNotification<T extends GlobalState>(
  global: T
) {
  ...
  return serviceNotifications.find(
    ({ id, isDeleted }) => !isDeleted && id === maxId
  );
}

export function selectIsMessageProtected<T extends GlobalState>(
  global: T,
  message?: ApiMessage
) {
  return Boolean(
    message &&
      (message.isProtected || selectIsChatProtected(global, message.chatId))
  );
}

export function selectIsChatProtected<T extends GlobalState>(
  global: T,
  chatId: string
) {
...
export function selectHasProtectedMessage<T extends GlobalState>(
  global: T,
  chatId: string,
  messageIds?: number[]
) {
...
export function selectCanForwardMessages<T extends GlobalState>(
  global: T,
  chatId: string,
  messageIds?: number[]
) {
...
export function selectSponsoredMessage<T extends GlobalState>(
  global: T,
  chatId: string
) {
  const chat = selectChat(global, chatId);
  const message =
    chat && isChatChannel(chat)
      ? global.messages.sponsoredByChatId[chatId]
      : undefined;

  return message && message.expiresAt >= Math.round(Date.now() / 1000)
    ? message
    : undefined;
}

export function selectDefaultReaction<T extends GlobalState>(
  global: T,
  chatId: string
) {
...
export function selectMaxUserReactions<T extends GlobalState>(
  global: T
): number {
  const isPremium = selectIsCurrentUserPremium(global);
  const { maxUserReactionsPremium = 3, maxUserReactionsDefault = 1 } =
    global.appConfig || {};
...
export function selectVisibleUsers<T extends GlobalState>(
  ...
  const messageIds = selectTabThreadParam(
    global,
    chatId,
    threadId,
    'viewportIds',
...
export function selectCanScheduleUntilOnline<T extends GlobalState>(
  global: T,
  id: string
) {
  ...
  return Boolean(
    !isChatWithSelf &&
      !chatBot &&
      isUserId(id) &&
      selectUserStatus(global, id)?.wasOnline
  );
}
...
export function selectMessageCustomEmojiSets<T extends GlobalState>(
  ...
  const documents = customEmojis.map(
    (entity) => global.customEmojis.byId[entity.documentId]
  );
...
export function selectForwardsContainVoiceMessages<T extends GlobalState>(
  ...
  const { messageIds, fromChatId } = selectTabState(
    global,
    tabId
  ).forwardMessages;
...
export function selectMessageTranslations<T extends GlobalState>(
  ...
  return (
    selectChatTranslations(global, chatId)?.byLangCode[toLanguageCode] || {}
  );
}
...
export function selectRequestedTranslationLanguage<T extends GlobalState>(
  ...
  return selectTabState(global, tabId).requestedTranslations.byChatId[chatId]
    ?.manualMessages?.[messageId];
}
...
export function selectMessagesWithCall<T extends GlobalState>(
  global: T,
  missed?: boolean
) {
  let messageWithCall: ApiMessage[] = [];

  for (const chatMessages of Object.values(global.messages.byChatId)) {
    const { byId } = chatMessages;
    Object.values(byId).forEach((message) => {
      if (message.content.action?.phoneCall) {
        messageWithCall.push(message);
      }
    });
  }

  return messageWithCall.sort((a, b) => b.id - a.id);
}
