  ...
  switch (update['@type']) {
    case 'newMessage': {
      ...
      if (
        ...
        newMessage.replyToMsgId
      ) {
        actions.loadTopicById({ chatId, topicId: newMessage.replyToMsgId });
      }
    ...
    case 'updateMessage': {
      ...
      global = updateFeedMessage(global, chatId, message.id!, message);
    ...
    case 'updateMessageSendSucceeded': {
      ...
      global = {
        ...global,
        fileUploads: {
          byMessageLocalId: omit(global.fileUploads.byMessageLocalId, [
            localId.toString(),
          ]),
        },
      };
      ...
      setGlobal(global);
      actions.focusNextReply({ tabId: getCurrentTabId() });
      break;
    }
    ...
    case 'updatePinnedIds': {
      ...
      Object.entries(updatePerThread).forEach(([threadId, ids]) => {
        ...
        const newPinnedIds = isPinned
          ? unique(pinnedIds.concat(ids)).sort((a: any, b: any) => b - a)
    ...
    case 'updateThreadInfo': {
      const { chatId, threadId, threadInfo, firstMessageId } = update;
      ...
      global = updateThreadInfo(
        ...
        chatId,
        threadId,
        newThreadInfo as ApiThreadInfo
      );
      ...
      if (
        ...
        threadInfo.lastReadInboxMessageId !==
          currentThreadInfo?.lastReadInboxMessageId
      ) {
          ...
          if (!oldResults.results || !pollUpdate.results.isMin) {
            ...
            newResults = {
              ...
              results: pollUpdate.results.results.map((result) => ({
                ...result,
                isChosen: oldResults.results!.find(
    ...
    case 'updateMessagePollVote': {
      ...
      options.forEach((option) => {
        const targetOptionIndex = newResults.findIndex(
          (result) => result.option === option
        );
        const targetOption = newResults[targetOptionIndex];
        const updatedOption: ApiPollResult = targetOption
          ? { ...targetOption }
          : { option, votersCount: 0 };
    ...
    case 'updateMessageExtendedMedia': {
      const { chatId, id, media, preview } = update;
      const message = selectChatMessage(global, chatId, id);
      const chat = selectChat(global, update.chatId);
...
function updateReactions<T extends GlobalState>(
  ...
  if (
    checkIfHasUnreadReactions(global, reactions) &&
    !alreadyHasUnreadReaction
  ) {
...
function updateListedAndViewportIds<T extends GlobalState>(
  ...
  const { threadInfo, firstMessageId } =
...
function updateChatLastMessage<T extends GlobalState>(
  ...
  if (currentLastMessage && message && !force) {
    const isSameOrNewer =
      currentLastMessage.id === message.id ||
      currentLastMessage.id === message.previousLocalId ||
