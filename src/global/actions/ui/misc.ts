...
addActionHandler(
  'toggleManagement',
  (global, actions, payload): ActionReturnType => {
    const { localChatId, tabId = getCurrentTabId() } = payload || {};
    const { chatId } = selectCurrentMessageList(global, tabId) || {};
    const curId = localChatId || chatId;
    if (!curId) {
      return undefined;
    }
      ...
      global,
      {
        management: {
          byChatId: {
            ...
            [curId]: {
              ...tabState.management.byChatId[curId],
              isActive: !(tabState.management.byChatId[curId] || {}).isActive,
            },
          },
        },
      },
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    ...
    const lastDelimiter = langProvider.translate(
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    const { shouldCompress, shouldSendGrouped } = payload;

    return {
      ...global,
      attachmentSettings: {
        shouldCompress:
          shouldCompress ?? global.attachmentSettings.shouldCompress,
        shouldSendGrouped:
          shouldSendGrouped ?? global.attachmentSettings.shouldSendGrouped,
      },
    };
  }
);
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    ...
    if (!currentChat)
      actions.openChat({ id: chatId, threadId: MAIN_THREAD_ID, tabId });
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    ...
    const leftScreen = selectTabState(global, tabId).leftScreen;
    if (leftScreen !== LeftColumnContent.ChatList) {
      switch (leftScreen) {
        case LeftColumnContent.Feed:
          setPageTitleInstant('Feed');
          break;
        case LeftColumnContent.Calls:
          setPageTitleInstant('Calls');
          break;
        case LeftColumnContent.Settings:
          setPageTitleInstant('Profile');
          break;
        case LeftColumnContent.Contacts:
          setPageTitleInstant('Contacts');
          break;
        default:
    ...
    if (
      ...
      if (currentChat) {
        const title = getChatTitle(t, currentChat, chatId === currentUserId);
