...
import { LoadMoreDirection, ThreadId } from '../../../types';
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    ...
    if (isSingle) {
      ...
      const { text, entities, attachments, replyingTo, ...commonParams } =
        params;
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    const { page: localPage, isExplore } = payload || {};
    const page = isExplore
      ? global.messages.inExplore.page!
      : global.messages.inFeed.page;

    const result = await callApi('fetchFeedMessages', {
      page: localPage || page,
      limit: sliceSize,
      isExplore,
    });
    ...
    global = getGlobal();
    global = isExplore
      ? addFeedExploreById(global, messages, localPage)
      : addFeedMessagesById(global, messages, localPage);

    setGlobal(global);
  }
);

addActionHandler('clearUnreadFeed', (global): ActionReturnType => {
  global = {
    ...global,
    messages: {
      ...global.messages,
...
addActionHandler(
  'saveDraft',
  async (global, actions, payload): Promise<void> => {
    const { chatId, threadId, draft, shouldForce } = payload;
    if (!draft) {
      return;
    }
    ...
    global = updateChat(global, chatId, {
      draftDate: Math.round(Date.now() / 1000),
    });
...
addActionHandler('clearDraft', (global, actions, payload): ActionReturnType => {
  const { chatId, threadId = MAIN_THREAD_ID, localOnly, shouldForce } = payload;
  ...
  const newDraft: ApiDraft | undefined = shouldForce
    ? { shouldForce, text: '' }
    : undefined;
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    ...
    void callApi('deleteMessages', { messageIds, shouldDeleteForAll });
  }
);
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    await callApi('deleteHistory', { chat, shouldDeleteForAll, isClear });

    const activeChat = selectCurrentMessageList(global, tabId);

    if (activeChat && !isClear && activeChat.chatId === chatId) {
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    const data: ApiError | ApiConfirm = result
      ? { success: 'ReportPeer.ReportConfirm' }
      : {
          message:
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    const webPagePreview = await callApi('fetchWebPagePreview', {
      message: text,
    });
      ...
      global,
      {
        pollResults: {
          ...
          voters: {
            ...voters,
            [option]: unique([
              ...(!shouldResetVoters && voters && voters[option]
                ? voters[option]
                : []),
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    const originChannelId = selectThreadOriginChat(
      global,
      chatId,
      threadId
    )?.id;
...
async function sendMessage<T extends GlobalState>(
  ...
  let localId: MessageKey | undefined;

  const progressCallback = params.attachment
    ? (progress: number, messageLocalId: MessageKey) => {
  ...
  if (!params.replyingTo && params.currentThreadId !== MAIN_THREAD_ID) {
    params.replyingTo = selectThreadTopMessageId(
      global,
      params.chat.id,
  ...
  if (
    params.replyingTo &&
    !params.replyingToTopId &&
    params.currentThreadId !== MAIN_THREAD_ID
  ) {
    params.replyingToTopId = selectThreadTopMessageId(
      global,
      params.chat.id,
      params.currentThreadId
    )!;
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    ...
    const unreadMentions = (chat.unreadMentions || []).filter(
...
addActionHandler('openUrl', (global, actions, payload): ActionReturnType => {
  ...
  if (urlWithProtocol.match(RE_TME_LINK) || urlWithProtocol.match(RE_TG_LINK)) {
    actions.openElloLink({ url, tabId });
    return;
  }
  ...
  const shouldDisplayModal =
    !urlWithProtocol.match(RE_ELLO_LINK) && !shouldSkipModal;
...
addActionHandler(
  'forwardContact',
  async (global, actions, payload): Promise<void> => {
    const { chatId, contactRequest, tabId = getCurrentTabId() } = payload;
    global = getGlobal();
    global = updateTabState(
      global,
      {
...
addActionHandler(
  'forwardReferralCode',
  async (global, actions, payload): Promise<void> => {
    const { chatId, refCode, tabId = getCurrentTabId() } = payload;
    global = getGlobal();
    global = updateTabState(
      global,
      {
...
addActionHandler('setLike', async (global, actions, payload): Promise<void> => {
  const { user_id, msg_id, isLiked, chatId, likes } = payload;
  let newLikes = likes || 0;

  const result = isLiked
    ? await callApi('subLike', { user_id, msg_id, chatId })
    : await callApi('addLike', { user_id, msg_id, chatId });

  if (!result) return;
  //console.log(user_id, msg_id, isLiked, chatId, likes);
  // global = getGlobal();
  // global = updateFeedMessage(global, chatId, msg_id, {
  //   isLiked: !isLiked,
  //   likes: isLiked ? newLikes-- : newLikes++,
  // });
  // setGlobal(global);
});
