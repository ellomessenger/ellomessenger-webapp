...
import type { ApiChat, ApiMessage, ApiThreadInfo } from '../../../api/types';
...
addActionHandler('sync', (global, actions): ActionReturnType => {
  ...
  const { loadAllChats, preloadTopChatMessages, getFeedFilter } = actions;
  ...
  loadAllChats({
    ...
    onReplace: async () => {
      ...
      const { chats, currentUserId } = global;

      if (currentUserId && !chats.byId[currentUserId]) {
...
async function loadAndReplaceMessages<T extends GlobalState>(
  ...
  for (const { id: tabId } of Object.values(global.byTabId)) {
    ...
    if (currentChatId && currentChat) {
      const result = await loadTopMessages(
        currentChat,
        activeThreadId,
        ...
        const localMessages =
          currentChatId === SERVICE_NOTIFICATIONS_USER_ID
            ? global.serviceNotifications
                ...
                .map(({ message }) => message)
            : [];
        const topicLastMessages =
          currentChat.isForum && currentChat.topics
            ? Object.values(currentChat.topics)
                .map(({ lastMessageId }) => currentChatMessages[lastMessageId])
                .filter(Boolean)
            : [];

        const allMessages = ([] as ApiMessage[]).concat(
          result.messages,
          localMessages
        );
