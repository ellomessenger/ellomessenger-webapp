...
const SERVICE_NOTIFICATIONS_USER_MOCK: ApiUser = {
  ...
  isBusiness: false,
  isPublic: false,
  title: 'Service notification user',
};
...
addActionHandler(
  ...
  async (global, actions): Promise<void> => {
    const preloadedChatIds = new Set<string>();
    const folderAllOrderedIds = getOrderedIds(ALL_FOLDER_ID);

    const newFolderAllOrderedIds =
      folderAllOrderedIds &&
      unique([
        ...folderAllOrderedIds.slice(0, TOP_CHAT_MESSAGES_PRELOAD_LIMIT),
        ...(global.feedFilter?.all || []),
      ]);

    for (let i = 0; i < (newFolderAllOrderedIds?.length || 0); i++) {
      ...
      const nextChatId = newFolderAllOrderedIds?.find(
        (id) => !currentChatIds.includes(id) && !preloadedChatIds.has(id)
      );
...
function abortChatRequestsForCurrentChat<T extends GlobalState>(
  ...
  if (
    currentChatId &&
    (currentChatId !== newChatId || currentThreadId !== newThreadId)
  ) {
    const [isChatOpened, isThreadOpened] = Object.values(global.byTabId).reduce(
      ([accHasChatOpened, accHasThreadOpened], { id: otherTabId }) => {
        ...
        const isSameThread =
          isSameChat && otherMessageList?.threadId === currentThreadId;

        return [
          accHasChatOpened || isSameChat,
          accHasThreadOpened || isSameThread,
        ];
      },
    ...
    if (shouldAbortChatRequests) {
      abortChatRequests(
        currentChatId,
        isChatOpened ? Number(currentThreadId) : undefined
      );
    }
  }
}
...
addActionHandler('openChat', (global, actions, payload): ActionReturnType => {
  ...
  if (currentChatId && (currentChatId !== id || currentThreadId !== threadId)) {
    ...
    if (shouldAbortChatRequests) {
      callApi('abortChatRequests', {
        chatId: currentChatId,
        threadId: isChatOpened ? Number(currentThreadId) : undefined,
      });
    }
  }
  ...
  if (chat && global.chats.listIds.bots?.includes(chat.id)) {
    void callApi('requestAiPurchases');
  }
});
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    if (threadId !== MAIN_THREAD_ID) {
      const topMessageId = selectThreadTopMessageId(global, chatId, threadId);
      ...
      if (!topMessageId) {
        ...
        const result = await callApi('requestThreadInfoUpdate', {
          chat,
          threadId,
          originChannelId,
        });
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    if (!isComments) {
      ...
      loadingThreadId = Number(threadId);
      ...
      if (threadId === MAIN_THREAD_ID) {
        ...
        originalChat?.isForum ||
        (chatId && getIsSavedDialog(chatId, threadId, global.currentUserId))
      ) {
        actions.processOpenChatOrThread({
          chatId,
          type,
    ...
    const threadOriginInfo = selectRepliesThreadInfo(
      global,
      loadingChatId,
      loadingThreadId
    );
    ...
    if (
      ...
      threadInfo?.threadId &&
      (isComments ||
        (thread?.listedIds?.length &&
          thread.listedIds.includes(Number(threadInfo.threadId))))
    ) {
    ...
    if (loadingThread) {
      abortChatRequests(
        ...
        loadingThread.loadingMessageId
      );
    }
    ...
    const openPreviousChat = () => {
      ...
      if (
        isComments ||
        selectCurrentMessageList(currentGlobal, tabId)?.chatId !==
          loadingChatId ||
        selectCurrentMessageList(currentGlobal, tabId)?.threadId !==
          loadingThreadId
      ) {
        return;
      }
    ...
    if (
      loadingThread?.loadingChatId !== loadingChatId ||
      loadingThread?.loadingMessageId !== loadingThreadId
    ) {
      openPreviousChat();
      return;
    }
    ...
    if (!result) {
      ...
      actions.showNotification({
        message: t(
          isComments ? 'Channel.PostDeleted' : 'lng_message_not_found'
        ),
        tabId,
      });
    ...
    if (isComments) {
      ...
      global = updateThreadInfo(global, chatId, threadId, {
        ...
        threadId,
        chatId,
        originChannelId: loadingChatId,
        ...(threadOriginInfo &&
          pick(threadOriginInfo, [
            'messagesCount',
            'lastMessageId',
    ...
    actions.loadViewportMessages({
      ...
      onError: () => {
        ...
        actions.showNotification({
          message: t('Group.ErrorAccessDenied'),
          tabId,
        });
      },
      onLoaded: () => {
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    const { chatId, threadId, messageId, tabId = getCurrentTabId() } = payload!;
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    const username = AI_BOT[id].username;
    if (username) {
    ...
    while (shouldReplace || !global.chats.isFullyLoaded[listType]) {
      ...
      const oldestChat = listIds
        ? listIds
            ...
            .sort(
              (chat1, chat2) => getOrderDate(chat1)! - getOrderDate(chat2)!
            )[0]
        : undefined;
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    const { chatId, force, tabId = getCurrentTabId(), withPhotos } = payload!;
    const chat = selectChat(global, chatId);
    if (!chat) {
      return;
    }
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    const users = (memberIds as string[])
      ...
      .filter(Boolean);
    const chatId = CREATE_NEW_PUBLIC_CHANNEL_ID;
    const username = selectTabState(global, tabId).management.byChatId[chatId]
      ?.checkedUsername;
    ...
    try {
      createdChannel = await callApi('createChannel', {
        title,
        about,
        users,
        payType,
      ...
      if ((error as ApiError).message === 'CHANNELS_TOO_MUCH') {
        ...
        actions.showDialog({
          data: { ...(error as ApiError), hasErrorKey: true },
          tabId,
        });
      }
    }
    ...
    global = updateTabState(
      global,
      {
        chatCreation: {
          ...
          progress: createdChannel
            ? ChatCreationProgress.Complete
            : ChatCreationProgress.Error,
        },
      },
      tabId
    );
    ...
    if (
      channelType &&
      username &&
      ['public', 'subscription', 'online'].includes(channelType)
    ) {
      await callApi('setChatUsername', {
        chat: createdChannel,
        username,
      });
    }
    ...
    if (channelType === 'online' || channelType === 'subscription') {
      void callApi('toggleIsProtected', {
        chat: createdChannel,
        isProtected: true,
      });
      // await callApi('toggleIsProtected', {
      //   chat: createdChannel,
      //   isProtected: true,
      // });
    }
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    const isPaid = isPaidChannel(chat);
    let isSubscribe;
    try {
      if (isPaid) {
        await callApi('paidSubscribe', {
          peer_id: Number(channelId.slice(1)),
          peer_type: 4,
        });
      } else {
        await callApi('joinChannel', { channelId, accessHash });
      }
      actions.openChat({ id: channelId, tabId });
      actions.requestChatUpdate({ chatId: channelId });
      if (isPaid) {
        actions.showDialog({
          data: {
            success: 'successfully subscribed',
            peer_type: isCourse ? 'course' : 'channel',
          },
          tabId,
        });
      }
      ...
      if ((error as ApiError).message === 'CHANNELS_TOO_MUCH') {
        ...
        (error as ApiError).message === 'not enough money for pay transaction'
      ) {
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    ...
    if (folderId) {
      ...
      const isPinned = selectIsChatPinned(
        global,
        id,
        listType === 'archived' ? ARCHIVED_FOLDER_ID : undefined
      );

      const ids =
        global.chats.orderedPinnedIds[
          listType === 'archived' ? 'archived' : 'active'
        ];
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    ...
    const pinned = feedFilter?.pinned || [];
    const { chatId } = payload;
    const checked = feedFilter?.pinned?.includes(chatId);
    actions.setFeedFilter({
      ...feedFilter,
      pinned: checked
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    const { feedFilter } = global;
    const hidden = feedFilter?.hidden || [];
    const { chatId } = payload;
    const checked = feedFilter?.hidden?.includes(chatId);
    actions.setFeedFilter({
      ...feedFilter,
      hidden: checked
        ? hidden.filter((id) => id !== chatId)
        : [...hidden, chatId],
    });
  }
);
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    ...
    const isLimitReached = idsToAdd.some(
      (id) => selectChatFolder(global, id)!.includedChatIds.length >= limit
    );
    if (isLimitReached) {
    ...
    idsToRemove.forEach(async (id) => {
      ...
      if (folder) {
        await callApi('editChatFolder', {
          ...
          folderUpdate: {
            ...folder,
            pinnedChatIds: folder.pinnedChatIds?.filter(
              (pinnedId) => pinnedId !== chatId
            ),
            includedChatIds: folder.includedChatIds.filter(
              (includedId) => includedId !== chatId
            ),
          },
        });
      }
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    if (!chat) {
      actions.openPreviousChat({ tabId });
      actions.showNotification({
        message: t('lng_username_by_phone_not_found').replace(
          '{phone}',
          phoneNumber
        ),
        tabId,
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    ...
    const hostname = TME_WEB_DOMAINS.has(uri.hostname)
      ? 'ello.team'
      : uri.hostname;
    const hostParts = hostname.split('.');
    if (hostParts.length > 3) return;
    const pathname =
      hostParts.length === 3 ? `${hostParts[0]}/${uri.pathname}` : uri.pathname;
    const [part1, part2, part3] = pathname
      .split('/')
      .filter(Boolean)
    ...
    const startAttach =
      params.hasOwnProperty('startattach') && !params.startattach
        ? true
        : params.startattach;
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    members?.push(kickedMembers?.find((u) => u.userId === user.id)!);
        ...
        ...(members &&
          !isBanned && {
            members: members.map((m) =>
              m.userId === userId ? { ...m, bannedRights } : m
            ),
          }),
        ...(isUnblocked &&
          kickedMembers && {
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    await callApi('editCreator', { chat, user });
    const chatAfterUpdate = await callApi('fetchFullChat', chat);
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    if (channel) {
      global = getGlobal();
      loadFullChat(global, actions, channel, tabId);
    }
  }
);
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    actions.setNewChatMembersDialogState({
      newChatMembersProgress: NewChatMembersProgress.Loading,
      tabId,
    });
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    if (
      !force &&
      chat.listedTopicIds &&
      chat.listedTopicIds.length === chat.topicsCount
    ) {
      return;
    }

    const offsetTopic =
      !force && chat.listedTopicIds
        ? chat.listedTopicIds.reduce((acc, el) => {
    ...
    const result = await callApi('fetchTopics', {
      ...
      offsetId,
      offsetDate,
      limit: offsetTopicId ? TOPICS_SLICE : TOPICS_SLICE_SECOND_LOAD,
    });
    ...
    Object.entries(result.readInboxMessageIdByTopicId || {}).forEach(
      ([topicId, messageId]) => {
        global = updateThreadInfo(global, chatId, Number(topicId), {
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    const result = await callApi('toggleParticipantsHidden', {
      chat,
      isEnabled,
    });
...
addActionHandler(
  ...
  (global, actions, payload): ActionReturnType => {
    ...
    if (
      isPinned &&
      Object.values(chat.topics).filter((topic) => topic.isPinned).length >=
        topicsPinnedLimit
    ) {
      actions.showNotification({
        message: t('LimitReachedPinnedTopics', { topicsPinnedLimit }),
        tabId,
      });
      return;
    }
...
addActionHandler('getFeedFilter', async (global): Promise<void> => {
  const result = await callApi('requestFeedFilter');
  if (!result) return;
  global = getGlobal();
  global = {
    ...global,
    feedFilter: {
      ...global.feedFilter,
      show_adult: false,
      show_only_subs: false,
      show_recommended: false,
      ...result,
      all: buildPeerId(result.all),
      hidden: buildPeerId(result.hidden),
      pinned: buildPeerId(result.pinned),
    },
  };
  global = updateFeedChatListIds(global, result);
...
addActionHandler(
  'setFeedFilter',
  (global, actions, payload): ActionReturnType => {
    const { pinned, hidden, show_adult, show_only_subs, show_recommended } =
      payload;
    void callApi('updateFeedFilter', {
      pinned: getInteger(pinned),
      hidden: getInteger(hidden),
      show_adult,
      show_only_subs,
      show_recommended,
    });
...
function getInteger(arr: string[] | undefined) {
  if (arr) {
    return arr.map((id) => Number(id.slice(1)));
  }
}
...
async function loadChats(
  ...
  if (shouldReplace && listType === 'active') {
    ...
    global = replaceUsers(
      global,
      buildCollectionByKey(visibleUsers.concat(result.users), 'id')
    );
    global = replaceUserStatuses(global, result.userStatusesById);
    global = replaceChats(
      global,
      buildCollectionByKey(visibleChats.concat(result.chats), 'id')
    );
  ...
  const idsToUpdateDraft = isFullDraftSync
    ? result.chatIds
    : Object.keys(result.draftsById);
  ...
  const idsToUpdateReplyingToId = isFullDraftSync
    ? result.chatIds
    : Object.keys(result.replyingToById);
  ...
  setGlobal(global);
  if (global.currentUserId && !global.chats.byId[global.currentUserId]) {
    void callApi('fetchChat', { type: 'self' });
  }
}
...
export async function loadFullChat<T extends GlobalState>(
  ...
  const { users, userStatusesById, fullInfo, groupCall, membersCount } = result;
...
export async function fetchChatByPhoneNumber<T extends GlobalState>(
  global: T,
  phoneNumber: string
) {
  global = getGlobal();
  ...
  const { chat, user } =
    (await callApi('getChatByPhoneNumber', phoneNumber)) || {};
  if (!chat) {
    return undefined;
  }
...
async function getAttachBotOrNotify<T extends GlobalState>(
  ...
  if (!result) {
    actions.showNotification({
      message: t('WebApp.AddToAttachmentUnavailableError'),
      tabId,
    });
...
async function openChatByUsername<T extends GlobalState>(
  ...
  const isCurrentChat = currentChat?.usernames?.some(
    (c) => c.username === username
  );

  const user = selectUserByUsername(global, username);
  const chat = await fetchChatByUsername(global, username);

  if (!chat) {
    if (!user?.isPublic) {
      actions.showNotification({
        message: t('Errors.contact_may_be_private'),
        tabId,
      });
    }
    return;
