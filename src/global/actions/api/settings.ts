...
import { selectUser, selectTabState } from '../../selectors';
import {
  addUsers,
  addBlockedContact,
  updateChats,
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    if (photo) {
      ...
      if (result) {
        ...
        actions.showNotification({
          message: t('Notification.UserPhotoChanged'),
          tabId: getCurrentTabId(),
        });
      }
    } else {
      const result = await callApi('changeProfile', {
        first_name,
        last_name,
        bio,
        username,
      ...
      if (result) {
        ...
        if (currentUser) {
          const shouldUsernameUpdate = currentUser.usernames?.find(
            (u) => u.isEditable
          );
          const usernames = shouldUsernameUpdate
            ? currentUser.usernames?.map((u) =>
                u.isEditable ? { ...u, username } : u
              )
            : [
                {
                  username,
        ...
        actions.showNotification({
          message: t('Notification.UserDataChanged'),
          tabId: getCurrentTabId(),
        });
      }
    }
    ...
    //   const result = await callApi('updateUsername',  user_name);
    //   global = getGlobal();
    //   const currentUser = currentUserId && selectUser(global, currentUserId);
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    if (
      ...
      tabState.profileEdit.progress === ProfileEditProgress.InProgress
    ) {
      return;
    }
    ...
    global = updateTabState(
      ...
      {
        profileEdit: {
          progress: tabState.profileEdit
            ? tabState.profileEdit.progress
            : ProfileEditProgress.Idle,
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    const { result, error } = (await callApi('checkChatUsername', {
      username,
    }))!;
    ...
    global = {
      ...global,
      authErrorAvailable: { ...global.authErrorAvailable, username: result },
    };
    setGlobal(global);
  }
);
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    const result = (await callApi('checkEmail', { email }))!;
    global = getGlobal();
    global = {
      ...global,
      authErrorAvailable: { ...global.authErrorAvailable, email: result },
    };
    setGlobal(global);
  }
);
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    const result = await callApi('updateNotificationSettings', peerType, {
      isSilent,
      shouldShowPreviews,
    });
    if (!result) {
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    const rules = buildInputPrivacyRules(global, {
      ...
      allowedIds: isAllowList
        ? contactsIds
        : [...settings.allowUserIds, ...settings.allowChatIds],
      deniedIds: !isAllowList
        ? contactsIds
        : [...settings.blockUserIds, ...settings.blockChatIds],
    });
...
function buildInputPrivacyRules(
  global: GlobalState,
  {
    visibility,
    allowedIds,
  ...
  if (visibility === 'contacts' || visibility === 'nobody') {
    users = allowedIds
      .map(collectUsers)
      .filter(Boolean) as InputPrivacyContact[];
    chats = allowedIds
      .map(collectChats)
      .filter(Boolean) as InputPrivacyContact[];
...
addActionHandler('getCategoriesForChannel', async (global): Promise<void> => {
  if (global.channelCategories.length) return;
  const response = await callApi('fetchCategoriesForChannel');
  if (!response) return;
  global = getGlobal();
  global = {
    ...global,
    channelCategories: response.categories.sort(),
  };
  setGlobal(global);
});

addActionHandler('getGenresForChannel', async (global): Promise<void> => {
  if (global.channelGenres.length) return;
  const response = await callApi('fetchGenresForChannel');
  if (!response) return;

  global = getGlobal();
  global = {
    ...global,
    channelGenres: response.genres,
  };
  setGlobal(global);
});
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    if (global.authNearestCountry) {
      const timeFormat = COUNTRIES_WITH_12H_TIME_FORMAT.has(
        global.authNearestCountry.toUpperCase()
      )
        ? '12h'
        : '24h';
    ...
    if (nearestCountryCode) {
      const timeFormat = COUNTRIES_WITH_12H_TIME_FORMAT.has(
        nearestCountryCode.toUpperCase()
      )
        ? '12h'
        : '24h';
      actions.setSettingOption({ timeFormat, tabId });
...
addActionHandler('loadGlobalPrivacySettings', async (global): Promise<void> => {
  ...
  global = replaceSettings(global, {
    shouldArchiveAndMuteNewNonContact:
      globalSettings.shouldArchiveAndMuteNewNonContact,
  });
  setGlobal(global);
});
...
addActionHandler(
  ...
  async (global, actions, payload = 2): Promise<void> => {
    const result = await callApi('fetchPaidSubscriptions', payload);
    if (result && result.items) {
      global = getGlobal();
      global = { ...global, subscriptionsList: result.items };
      setGlobal(global);
    }
  }
);
...
addActionHandler(
  ...
  (global, action, payload): ActionReturnType => {
    ...
    void callApi('sendVerificationCode', { ...payload, tabId });
  }
);

addActionHandler(
  'changeEmail',
  async (global, action, payload): Promise<void> => {
    const result = await callApi('changeEmail', payload);
    if (result && result.status) {
      action.clearConfirmEmail();
...
addActionHandler(
  'changePassword',
  async (global, action, payload): Promise<void> => {
    const result = await callApi('changePassword', payload);
...
addActionHandler(
  'getConfirmCodeFromDeleteAccount',
  (global, action, payload): ActionReturnType => {
    const tabId = getCurrentTabId();
    void callApi('verifyEmailInPassword', { ...payload, tabId });
  }
);
...
addActionHandler(
  'deleteAccount',
  async (global, action, payload): Promise<void> => {
    const result = await callApi('deleteAccount', payload);
    if (result) {
      action.signOut({ forceInitApi: true });
      action.showNotification({
        type: 'success',
        message: result.message,
        tabId: getCurrentTabId(),
...
addActionHandler('getInfoBeforeDeletion', async (global): Promise<void> => {
  const result = await callApi('requestBeforeDeletion');
  if (result) {
    global = getGlobal();
    global = {
      ...global,
      accountDeleteInfo: result,
    };
    setGlobal(global);
  }
});

addActionHandler('clearConfirmEmail', (global): ActionReturnType => {
  global = getGlobal();
  global = { ...global, confirmData: undefined };
  setGlobal(global);
});

addActionHandler('setAiPurchases', (global): ActionReturnType => {
...
//   'toggleChatUsername',
//   async (global, actions, payload): Promise<void> => {
//     const { chatId, username, isActive, tabId = getCurrentTabId() } = payload;
