...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    const { chats, users, messages, totalCount, nextOffsetId } = result;
    ...
    global = updateLocalTextSearchResults(
      global,
      chat.id,
      threadId,
      newFoundIds,
...
addActionHandler(
  ...
  async (global, actions, payload): Promise<void> => {
    ...
    // = await callApi('findFirstMessageIdAfterDate', {
    //   chat,
    //   timestamp,
    // });
