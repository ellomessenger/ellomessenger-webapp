...
export function createWindow(url?: string) {
  ...
  const window = new BrowserWindow({
    ...
    width,
    height,
    title: getAppTitle('Ello Beta'),
    icon: path.join(__dirname, '../public/icon-electron-windows.ico'),
    webPreferences: {
...
export function setupElectronActionHandlers() {
  ...
  ipcMain.handle(
    ElectronAction.SET_TRAFFIC_LIGHT_POSITION,
    (_, position: TrafficLightPosition) => {
      if (!IS_MAC_OS) {
        return;
      }

      getCurrentWindow()?.setTrafficLightPosition(
  ...
  ipcMain.handle(
    ...
    async (_, isAutoUpdateEnabled: boolean) => {
      if (IS_PREVIEW) {
        return;
      }
