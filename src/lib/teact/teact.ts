...
// eslint-disable-next-line @typescript-eslint/naming-convention
const DEBUG_memos: Record<
  string,
  { key: string; calls: number; misses: number; hitRate: number }
> = {};
...
document.addEventListener('dblclick', () => {
  ...
  console.warn(
    'COMPONENTS',
    orderBy(
      Object.values(DEBUG_components).map(({ avgRenderTime, ...state }) => {
        return {
          ...state,
  ...
  safeExec(
    () => {
      ...
      if (DEBUG) {
        ...
        DEBUG_components[componentName].avgRenderTime =
          (avgRenderTime * renders + duration) / (renders + 1);
...
export function hasElementChanged($old: VirtualElement, $new: VirtualElement) {
  if (typeof $old !== typeof $new) {
    ...
    $old.type === VirtualType.Component &&
    ...
    byCursor[cursor] = {
      ...
      setter: (newValue: ((current: T) => T) | T) => {
        ...
        if (typeof newValue === 'function') {
          newValue = (newValue as (current: T) => T)(
...
function useEffectBase(
  ...
  const runEffectCleanup = () =>
    safeExec(
      () => {
        ...
        if (DEBUG) {
          const duration = performance.now() - DEBUG_startAt!;
          const componentName = DEBUG_resolveComponentName(
      ...
      () => {
        // eslint-disable-next-line no-console
        console.error(
          `[Teact] Error in effect cleanup at cursor #${cursor} in ${componentInstance.name}`
        );
      },
      () => {
  ...
  const runEffect = () =>
    safeExec(
      ...
      () => {
        // eslint-disable-next-line no-console
        console.error(
          `[Teact] Error in effect at cursor #${cursor} in ${componentInstance.name}`
        );
      }
    );
  ...
  function setupSignals() {
    const cleanups = dependencies?.filter(isSignal).map((signal, i) =>
      signal.subscribe(() => {
        if (debugKey) {
          // eslint-disable-next-line no-console
          console.log(
            `[Teact] Effect "${debugKey}" caused by signal #${i} new value:`,
            signal()
          );
        }
...
export function useMemo<T extends any>(
  ...
  if (DEBUG && debugHitRateKey) {
    ...
    DEBUG_state.calls++;
    DEBUG_state.hitRate =
      (DEBUG_state.calls - DEBUG_state.misses) / DEBUG_state.calls;
  }
      ...
      if (DEBUG_state) {
        ...
        if (
          ...
          console.warn(
            ...
            `[Teact] ${
              DEBUG_state.key
            }: Hit rate is ${DEBUG_state.hitRate.toFixed(2)} for ${
              DEBUG_state.calls
            } calls`
          );
        }
...
export function memo<T extends FC_withDebug>(Component: T, debugKey?: string) {
  ...
  TeactMemoWrapper.DEBUG_contentComponentName =
    DEBUG_resolveComponentName(Component);
...
export function DEBUG_resolveComponentName(Component: FC_withDebug) {
  ...
  return (
    name + (DEBUG_contentComponentName ? `>${DEBUG_contentComponentName}` : '')
  );
}
