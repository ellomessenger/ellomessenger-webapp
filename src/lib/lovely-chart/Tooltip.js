...
export function createTooltip(
  ...
  function _setupBalloon() {
    ...
    _balloon.className = `lovely-chart--tooltip-balloon${
      !data.isZoomable ? ' lovely-chart--state-inactive' : ''
    }`;
    _balloon.innerHTML =
  ...
  function _getLabelIndex() {
    ...
    return labelIndex < _state.labelFromIndex ||
      labelIndex > _state.labelToIndex
      ? null
      : labelIndex;
  }
  ...
  function _selectLabel(isExternal) {
    ...
    const shouldShowBalloon = data.isPie
    ...
    function getValue(values, labelIndex) {
      if (data.isPie) {
        return values
          .slice(_state.labelFromIndex, _state.labelToIndex + 1)
          .reduce((a, x) => a + x, 0);
      }
    ...
    if (data.isLines || data.isAreas) {
      ...
      _drawTail(
        xPx,
        plotSize.height - X_AXIS_HEIGHT,
        getCssColor(colors, 'grid-lines')
      );
    }
  }
  ...
  function _drawCircles(statistics, labelIndex) {
    statistics.forEach(({ value, key, hasOwnYAxis, originalIndex }) => {
      ...
      const point = hasOwnYAxis
        ? _secondaryPoints[pointIndex]
        : _points[originalIndex][pointIndex];
  ...
  function _getBalloonLeftOffset(labelIndex) {
    ...
    const shouldPlaceRight = data.isPie
      ? angle > Math.PI / 2
      : labelIndex < meanLabel;
    ...
    return Math.min(
      ...
      container.offsetWidth - _balloon.offsetWidth
    );
  }
  ...
  function _updateBalloon(statistics, labelIndex) {
    _balloon.style.transform = `translate3D(${_getBalloonLeftOffset(
      labelIndex
    )}px, ${_getBalloonTopOffset()}, 0)`;
  ...
  function _getTitle(data, labelIndex) {
    switch (data.tooltipFormatter) {
      ...
      default:
        return labelIndex ? data.xLabels[labelIndex].text : '';
    }
  }

  function _isPieSectorSelected(
    statistics,
    value,
    totalValue,
    index,
    ...
    const offset =
      index > 0
        ? statistics.slice(0, index).reduce((a, x) => a + x.value, 0)
        : 0;
    const beginAngle = (offset / totalValue) * Math.PI * 2 - Math.PI / 2;
    const endAngle =
      ((offset + value) / totalValue) * Math.PI * 2 - Math.PI / 2;
  ...
  function _updateTitle(title) {
    ...
    if (data.isPie) {
      ...
      const currentTitle = titleContainer.querySelector(
  ...
  function _updateDataSet(currentDataSet, { key, value } = {}, totalValue) {
    ...
    const valueElement = currentDataSet.querySelector(
      `.lovely-chart--tooltip-dataset-value.lovely-chart--color-${data.colors[
        key
      ].slice(1)}:not(.lovely-chart--state-hidden)`
    );
  ...
  function _renderPercentageValue(dataSet, value, totalValue) {
    ...
    if (data.isPie) {
      Array.from(
        dataSet.querySelectorAll(`.lovely-chart--percentage-title`)
      ).forEach((e) => e.remove());
      return;
    }

    const percentageValue = totalValue
    ...
    if (!percentageElement) {
      ...
      newPercentageTitle.className =
        'lovely-chart--percentage-title lovely-chart--position-left';
  ...
  function _updateDataSets(statistics) {
    ...
    Array.from(dataSetContainer.children).forEach((dataSet) => {
      if (
        !data.isPie &&
        dataSetContainer.classList.contains('lovely-chart--tooltip-legend-pie')
      ) {
        dataSet.remove();
      } else {
    ...
    finalStatistics.forEach((statItem) => {
      const currentDataSet = dataSetContainer.querySelector(
        `[data-name="${statItem.name}"]`
      );
    ...
    Array.from(
      dataSetContainer.querySelectorAll('[data-present="false"]')
    ).forEach((dataSet) => {
      dataSet.remove();
    });
  }
  ...
  function getPointerVector() {
    ...
    const distance = Math.sqrt(
      (_offsetX - center[0]) ** 2 + (_offsetY - center[1]) ** 2
    );
