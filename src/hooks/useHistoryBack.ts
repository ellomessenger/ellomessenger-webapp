...
function deferHistoryOperation(historyOperation: HistoryOperation) {
  if (!deferredHistoryOperations.length)
    fastRaf(applyDeferredHistoryOperations);
...
function resetHistory() {
  ...
  if (
    HISTORY_STATE &&
    HISTORY_STATE.leftScreen === LeftColumnContent.ChatList
  ) {
...
export default function useHistoryBack({
  ...
  const pushState = useCallback(
    (forceReplace = false) => {
      const isChatId =
        HISTORY_STATE.hash! &&
        typeof Number(HISTORY_STATE.hash.slice(1)) === 'number';
      ...
      deferHistoryOperation({
        ...
        data: {
          index: indexRef.current,
          historyUniqueSessionId,
          ...(HISTORY_STATE?.leftScreen && {
            leftScreen: HISTORY_STATE.leftScreen,
          }),
          ...(HISTORY_STATE?.middleScreen && {
            middleScreen: HISTORY_STATE.middleScreen,
          }),
          hash: hash ? `#${hash}` : ' ',
        },
  ...
  useSyncEffect(
    ([prevIsActive]) => {
      //console.log(prevIsActive, isActive, page);

      if (prevIsActive === isActive) return;

      if (isFirstRender.current && !isActive) return;

      if (isActive && !withoutPush) {
        pushState();
      } else {
        processBack();
      }
