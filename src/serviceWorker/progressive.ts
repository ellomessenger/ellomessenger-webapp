...
export async function respondForProgressive(e: FetchEvent) {
  ...
  const [cachedArrayBuffer, cachedHeaders] = !MEDIA_PROGRESSIVE_CACHE_DISABLED
    ? await fetchFromCache(cacheKey)
    : [];

  if (DEBUG) {
    // eslint-disable-next-line no-console
    console.log(
      `FETCH PROGRESSIVE ${cacheKey} (request: ${start}-${originalEnd}) CACHED: ${Boolean(
        cachedArrayBuffer
      )}`
    );
  }
...
async function fetchFromCache(cacheKey: string) {
  ...
  return Promise.all([
    ...
    cache
      .match(`${cacheKey}&type=headers`)
      .then((r) => (r ? r.json() : undefined)),
  ]);
}
...
export async function requestPart(
  ...
  const client = isDownload
    ? (await self.clients.matchAll()).find(
        (c) => c.type === 'window' && c.frameType === 'top-level'
      )
  ...
  const promise = Promise.race([
    pause(PART_TIMEOUT).then(() =>
      isResolved ? undefined : Promise.reject(new Error('ERROR_PART_TIMEOUT'))
    ),
