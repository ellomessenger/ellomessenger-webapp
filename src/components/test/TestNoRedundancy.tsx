...
const TestB: FC<BStateProps & BOwnProps> = ({
  bValue,
  aValue,
  derivedAValue,
}) => {
  ...
  return (
    <div className='TestB'>
      ...
      <div>derivedAValue = {derivedAValue}</div>
      {bValue > 0.5 ? (
        <span key='hello' className='hello'>
          Hello
        </span>
      ) : (
        <span key='world' className='world'>
          World
        </span>
      )}
    </div>
...
export default withGlobal((global): AStateProps => {
  // @ts-ignore
  // eslint-disable-next-line no-console
