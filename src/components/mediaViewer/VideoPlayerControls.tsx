...
const VideoPlayerControls: FC<OwnProps> = ({
  ...
  const handleSeek = useCallback(
    (e: MouseEvent | TouchEvent) => {
      if (isSeekingRef.current && seekerRef.current) {
        ...
        onSeek(
          Math.max(Math.min(duration * ((clientX - left) / width), duration), 0)
        );
      }
    },
    [duration, onSeek]
  ...
  return (
    <div
      className={classNames('VideoPlayerControls', {
    ...
    >
      ...
      <Menu
        ...
        className={classNames('playback-rate-menu', {
          'no-fullscreen': !isFullscreenSupported,
          'no-pip': !isPictureInPictureSupported,
        })}
        positionX='right'
        positionY='bottom'
        autoClose
...
function renderSeekLine(
  ...
  duration: number,
  bufferedRanges: BufferedRange[],
  seekerRef: React.RefObject<HTMLDivElement>
) {
