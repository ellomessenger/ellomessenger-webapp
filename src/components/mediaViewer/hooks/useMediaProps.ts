...
export const useMediaProps = ({
  ...
  const isPhoto = Boolean(
    !isVideo && (photo || webPagePhoto || isDocumentPhoto || actionPhoto)
  );
  const { isGif } = video || webPageVideo || {};
  const isFromSharedMedia = origin === MediaViewerOrigin.SharedMedia;
  const isFromSearch = origin === MediaViewerOrigin.SearchResult;

  const getMediaHash = useMemo(
    () => (isFull?: boolean) => {
      if (avatarOwner) {
        if (avatarMedia) {
      ...
      return (
        message && getMessageMediaHash(message, isFull ? 'full' : 'preview')
      );
    },
  ...
  const { mediaData: fullMediaBlobUrl, loadProgress } =
  ...
  let bestImageData =
    (!isVideo && (localBlobUrl || fullMediaBlobUrl)) ||
    previewBlobUrl ||
    pictogramBlobUrl;
  const thumbDataUri = useBlurSync(
    !bestImageData && message && getMessageMediaThumbDataUri(message)
  );
  ...
  const bestData =
    ...
    fullMediaBlobUrl ||
    (!isVideo
      ? previewBlobUrl || pictogramBlobUrl || bestImageData
      : undefined);
  ...
  if (message) {
    if (isDocumentPhoto || isDocumentVideo) {
      ...
      dimensions = getPhotoFullDimensions(
        (photo || webPagePhoto || actionPhoto)!
      )!;
    } else if (video || webPageVideo) {
      dimensions = getVideoDimensions((video || webPageVideo)!)!;
    }
  } else {
    dimensions = isVideoAvatar
      ? VIDEO_AVATAR_FULL_DIMENSIONS
      : AVATAR_FULL_DIMENSIONS;
  }
