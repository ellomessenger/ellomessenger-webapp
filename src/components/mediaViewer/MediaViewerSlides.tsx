...
const MediaViewerSlides: FC<OwnProps> = ({
  ...
  const [activeMediaId, setActiveMediaId] = useState<number | undefined>(
    mediaId
  );
  const prevZoomLevelChange = usePrevious(zoomLevelChange);
  const hasZoomChanged =
    prevZoomLevelChange !== undefined &&
    prevZoomLevelChange !== zoomLevelChange;
  ...
  const selectMediaDebounced = useDebouncedCallback(
    selectMedia,
    [selectMedia],
    DEBOUNCE_MESSAGE,
    true
  ...
  const setIsActiveDebounced = useDebouncedCallback(
    (value: boolean) => {
  ...
  useEffect(() => {
    ...
    const calculateOffsetBoundaries = (
      ...
      const minOffsetY = Math.max(
        -initialContentRect.top * scale + offsetTop,
        centerY
      );
    ...
    const onRelease = (e: MouseEvent | RealTouchEvent | WheelEvent) => {
      ...
      if (scale > 1) {
        ...
        let x1 =
          x * scaleFactor + (lastZoomCenter.x - scaleFactor * lastZoomCenter.x);
        let y1 =
          y * scaleFactor + (lastZoomCenter.y - scaleFactor * lastZoomCenter.y);
        ...
        if (e.type !== 'wheel' && lastTransform.scale === scale) {
          ...
          const Vx =
            Math.abs(lastDragOffset.x) / (Date.now() - lastGestureTime);
          const Vy =
            Math.abs(lastDragOffset.y) / (Date.now() - lastGestureTime);
    ...
    const cleanup = captureEvents(containerRef.current, {
      ...
      onDrag: (
        ...
        if (IS_IOS && captureEvent.type === 'touchstart') {
          ...
          if (
            pageX <= IOS_SCREEN_EDGE_THRESHOLD ||
            pageX >= windowWidth - IOS_SCREEN_EDGE_THRESHOLD
          ) {
            return;
          }
        }
        ...
        if (swipeDirectionRef.current !== SwipeDirection.Vertical) {
          ...
          if (
            ...
            absOffsetX / absOffsetY > tolerance
          ) {
            swipeDirectionRef.current = SwipeDirection.Horizontal;
            isActiveRef.current = false;
            ...
            if (
              event.type === 'wheel' &&
              Math.abs(x1) > SWIPE_X_THRESHOLD * 2
            ) {
              onRelease(event);
        ...
        if (
          ...
          absOffsetY / absOffsetX > tolerance
        ) {
          swipeDirectionRef.current = SwipeDirection.Vertical;
      ...
      onZoom: (
        ...
        const scale =
          zoom ??
          clamp(
            lastTransform.scale * zoomFactor!,
            MIN_ZOOM * 0.5,
            MAX_ZOOM * 3
          );
      ...
      onClick(e) {
        setIsMouseDown(false);
        const [isInThreshold, hasNextSlide] = changeSlideOnClick(
          e as MouseEvent
        );
        if (isInThreshold) {
          e.preventDefault();
        ...
        if (
          shouldCloseOnVideo ||
          !checkIfInsideSelector(e.target as HTMLElement, '.VideoPlayer')
        ) {
          onClose();
        }
      },
    ...
    <div className='MediaViewerSlides' ref={containerRef}>
      {hasPrev && scale === 1 && !isResizing && (
        <div
          ...
          style={getAnimationStyle(-windowWidth + offsetX - SLIDES_GAP)}
        >
          <MediaViewerContent
            {...rest}
            animationLevel={animationLevel}
      ...
      <div
        className={classNames('MediaViewerSlide', 'MediaViewerSlide--active', {
          'MediaViewerSlide--moving': isMoving,
        })}
...
function getAnimationStyle(x = 0, y = 0, scale = 1) {
  return {
    transform: `translate3d(${x.toFixed(3)}px, ${y.toFixed(
      3
    )}px, 0px) scale(${scale.toFixed(3)})`,
  };
}
