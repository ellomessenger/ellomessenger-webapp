...
const ChatFolders: FC<OwnProps & StateProps> = ({
  ...
  const { loadChatFolders, setActiveChatFolder, openChat } = getActions();
    ...
    return {
      ...
      title:
        orderedFolderIds?.[0] === ALL_FOLDER_ID
          ? t('ChatList.FilterAllChatsShort')
          : t('ChatList.FilterAllChats'),
    };
  }, [orderedFolderIds]);
  ...
  const isNotInFirstFolderRef = useRef(false);
  ...
  return (
    <div
      className={classNames('ChatFolders', {
        'ChatFolders--tabs-hidden': shouldRenderFolders && shouldHideFolderTabs,
      })}
    >
      {shouldRenderFolders ? (
        <TabList
...
export default memo(
  withGlobal<OwnProps>((global): StateProps => {
    const {
      chatFolders: { byId: chatFoldersById, orderedIds: orderedFolderIds },
      chats: {
        listIds: { archived },
      },
      currentUserId,
