...
const ForumPanel: FC<OwnProps & StateProps> = ({
  ...
  const orderedIds = useMemo(() => {
    return chat?.topics
      ? getOrderedTopics(
          Object.values(chat.topics),
          chat.orderedPinnedTopicIds
        ).map(({ id }) => id)
      : [];
  }, [chat]);

  const { orderDiffById, getAnimationType } = useOrderDiff(
    orderedIds,
    chat?.id
  );
  ...
  const [viewportIds, getMore] = useInfiniteScroll(
    ...
    orderedIds,
    !chat?.topicsCount || orderedIds.length >= chat.topicsCount,
    TOPICS_SLICE
  );
  ...
  useEffect(
    () => (isVisible ? captureEscKeyListener(handleClose) : undefined),
  ...
  useEffect(() => {
    ...
    return captureEvents(ref.current!, {
      ...
      onSwipe: (e, direction) => {
        const closeDirection = lang.isRtl
          ? SwipeDirection.Left
  ...
  return (
    ...
    >
      ...
      <InfiniteScroll
        ...
        beforeChildren={
          <div ref={scrollTopHandlerRef} className={styles.scrollTopHandler} />
        }
      >
      ...
      {!isLoading &&
        viewportIds?.length === 1 &&
        viewportIds[0] === GENERAL_TOPIC_ID && <EmptyForum chatId={chat.id} />}
    </div>
  );
};
