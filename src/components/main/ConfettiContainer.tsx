...
const ConfettiContainer: FC<StateProps> = ({ confetti }) => {
  ...
  const { lastConfettiTime, top, width, left, height } = confetti || {};

  const generateConfetti = useCallback(
    (w: number, h: number, amount = defaultConfettiAmount) => {
  ...
  const updateCanvas = useCallback(() => {
    ...
    confettiRef.current.forEach((c, i) => {
      ...
      const newFlicker =
        size * Math.abs(Math.sin(frameCount * flickerFrequency));
  ...
  if (
    !lastConfettiTime ||
    Date.now() - lastConfettiTime > CONFETTI_FADEOUT_TIMEOUT
  ) {
    return null;
  }
