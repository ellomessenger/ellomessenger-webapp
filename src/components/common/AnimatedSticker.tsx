...
import usePriorityPlaybackCheck, {
  isPriorityPlaybackActive,
} from '../../hooks/usePriorityPlaybackCheck';
import useSharedIntersectionObserver from '../../hooks/useSharedIntersectionObserver';
import { useStateRef } from '../../hooks/useStateRef';
import useSyncEffect from '../../hooks/useSyncEffect';
import useThrottledCallback from '../../hooks/useThrottledCallback';
import useUniqueId from '../../hooks/useUniqueId';
import useBackgroundMode, {
  isBackgroundModeActive,
} from '../../hooks/window/useBackgroundMode';
import { ApiReaction } from '../../api/types';
...
const AnimatedSticker: FC<OwnProps> = ({
  ...
  const init = useLastCallback(() => {
    if (
      ...
      (sharedCanvas &&
        (!sharedCanvasCoords ||
          !sharedCanvas.offsetWidth ||
          !sharedCanvas.offsetHeight))
    ) {
      return;
    }
  ...
  useHeavyAnimationCheck(
    pauseAnimation,
    playAnimation,
    !playKey || forceAlways || forceOnHeavyAnimation
  );
  usePriorityPlaybackCheck(
    pauseAnimation,
    playAnimation,
    !playKey || forceAlways
  );
  ...
  return (
    <div
      ...
      style={{
        ...
        cursor: onClick && !IS_ELECTRON ? 'pointer' : undefined,
        ...style,
      }}
      onClick={onClick}
    />
...
function isFrozen(forceAlways = false, forceOnHeavyAnimation = false) {
  ...
  return (
    (!forceOnHeavyAnimation && isHeavyAnimating()) ||
    isPriorityPlaybackActive() ||
    isBackgroundModeActive()
  );
}
