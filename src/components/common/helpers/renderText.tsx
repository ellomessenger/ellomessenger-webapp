...
import type { TeactNode, TextPart } from '../../../types';
    ...
    const splittenParts = part
      ...
      .reduce((parts: TextPart[], line: string, i, source) => {
        ...
        if (i !== source.length - 1) {
          parts.push(type === 'jsx' ? <br key={Math.random()} /> : '<br />');
        }
...
function addHighlight(
  ...
  return textParts.reduce<TextPart[]>((result, part) => {
    ...
    newParts.push(
      <span
        className={classNames('matching-text-highlight', {
          'is-quote': isQuote,
        })}
      >
  ...
  return textParts.reduce<TextPart[]>((result, part) => {
    ...
    while (nextLink) {
      ...
      if (nextLink.startsWith('@')) {
        ...
        content.push(<SafeLink key={index} text={nextLink} url={nextLink} />);
      }
