...
export const ROUND_VIDEO_DIMENSIONS_PX = 180;
...
const MESSAGE_MAX_WIDTH_REM = 24;
const MESSAGE_OWN_MAX_WIDTH_REM = 24;
const MESSAGE_FEED_MAX_WIDTH_REM = 26.5;
...
function getMaxMessageWidthRem(
  ...
  const regularMaxWidth = fromOwnMessage
    ? MESSAGE_OWN_MAX_WIDTH_REM
    : isFeed
    ? MESSAGE_FEED_MAX_WIDTH_REM
    : MESSAGE_MAX_WIDTH_REM;
  if (!isMobile && !noAvatars) {
  ...
  if (!cachedMaxWidthOwn) {
    cachedMaxWidthOwn = Math.min(
      isFeed ? MESSAGE_FEED_MAX_WIDTH_REM : MESSAGE_OWN_MAX_WIDTH_REM,
      windowWidth / REM - MOBILE_SCREEN_NO_AVATARS_MESSAGE_EXTRA_WIDTH_REM
    );
  }
  if (!cachedMaxWidth) {
    cachedMaxWidth = Math.min(
      isFeed ? MESSAGE_FEED_MAX_WIDTH_REM : MESSAGE_MAX_WIDTH_REM,
...
export function getAvailableWidth(
  ...
  const availableWidthRem =
    getMaxMessageWidthRem(fromOwnMessage, noAvatars, isMobile, isFeed) -
    extraPaddingRem;
...
export function getMediaViewerAvailableDimensions(
  withFooter: boolean,
  isVideo: boolean
): ApiDimensions {
...
export function calculateInlineImageDimensions(
  ...
  const { width, height } =
    getPhotoInlineDimensions(photo) || DEFAULT_MEDIA_DIMENSIONS;
...
export function calculateVideoDimensions(
  ...
  const { width, height } =
    getVideoDimensions(video) || DEFAULT_MEDIA_DIMENSIONS;
