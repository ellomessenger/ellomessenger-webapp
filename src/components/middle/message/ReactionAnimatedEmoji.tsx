...
const ReactionAnimatedEmoji = ({
  ...
  const { customEmoji } = useCustomEmoji(
    isCustom ? reaction.documentId : undefined
  );
  ...
  const assignedEffectId = useMemo(() => {
    ...
    const assignedId = availableReactions?.find(
      (available) => available.reaction.emoticon === customEmoji.emoji
    )?.aroundAnimation?.id;
    return assignedId;
  ...
  const shouldPlayEffect = Boolean(
    withEffects &&
      activeReaction &&
      (isCustom || mediaDataCenterIcon) &&
      mediaDataEffect
  );
  const shouldPlayCenter =
    isIntersecting && ((shouldPlayEffect && !withEffectOnly) || shouldLoop);
  const {
  ...
  const [isAnimationLoaded, markAnimationLoaded, unmarkAnimationLoaded] =
    useFlag();
  const shouldShowStatic =
    !isCustom && (!shouldPlayCenter || !isAnimationLoaded);
  const {
  ...
  return (
    <div style={{position: 'relative'}} id='your-block-id' className={rootClassName} ref={ref}>
      ...
      {effectReactin?.emoticon === reaction.emoticon && (
        <>
          <AnimatedSticker
            ...
            style={{ display: 'block', position: 'absolute', textAlign: 'center', top: '-20px', left: '-20px'}}
          />
          ...
          {isCustom && !assignedEffectId && isIntersecting && (
            <CustomEmojiEffect
              ...
              particleSize={Math.max(
                size * CUSTOM_EMOJI_EFFECT_MULTIPLIER,
                MIN_PARTICLE_SIZE
              )}
              onEnded={handleEnded}
            />
...
export default memo(
  withGlobal<OwnProps>((global, { containerId }) => {
    const { availableReactions, genericEmojiEffects } = global;
    const { activeReactions } = selectTabState(global);
    
    const withEffects = selectPerformanceSettingsValue(
      global,
      'reactionEffects'
    );
