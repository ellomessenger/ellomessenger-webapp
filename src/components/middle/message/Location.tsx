...
const Location: FC<OwnProps> = ({
  ...
  const secondsBeforeEnd =
    type === 'geoLive' && !isExpired
      ? message.date + location.period - serverTime
      : undefined;

  const [point, setPoint] = useState(geo);

  const shouldRenderText =
    type === 'venue' || (type === 'geoLive' && !isExpired);
  ...
  const handleClick = () => {
    ...
    openUrl({ url, shouldSkipModal: true });
  };
  ...
  const updateCountdown = useCallback(
    (countdownEl: HTMLDivElement) => {
      ...
      const text = formatCountdownShort(t, timeLeft * 1000);
      ...
      if (!svgEl || !timerEl) {
        ...
        svgEl.firstElementChild!.setAttribute(
          'stroke-dashoffset',
          `-${strokeDashOffset}`
        );
      }
    },
    [type, message.date, location]
  );
  ...
  useEffect(() => {
    ...
    if (
      Math.abs(geo.lat - point.lat) < MOVE_THRESHOLD &&
      Math.abs(geo.long - point.long) < MOVE_THRESHOLD
    ) {
    ...
    if (type === 'venue') {
      return (
        <div className='location-info'>
          <div className='location-info-title'>{location.title}</div>
          <div className='location-info-subtitle'>{location.address}</div>
        </div>
      );
    }
    if (type === 'geoLive') {
      return (
        <div className='location-info'>
          <div className='location-info-title'>{t('Attach.LiveLocation')}</div>
          <div className='location-info-subtitle'>
            {formatLastUpdated(t, serverTime, message.editDate)}
          </div>
  ...
  function renderMap() {
    ...
    return (
      <img
        ...
        style={{
          width: `${DEFAULT_MAP_CONFIG.width}px`,
          height: `${DEFAULT_MAP_CONFIG.height}px`,
        }}
      />
    );
  }
  ...
  function renderPin() {
    const pinClassName = classNames('pin', type, { expired: isExpired });

    if (type === 'geoLive') {
      return (
        <div className={pinClassName}>
          <PinSvg />
          <Avatar peer={avatarChat || avatarUser} className='location-avatar' />
    ...
    return <img className={pinClassName} src={mapPin} alt='' />;
  }
      ...
      <>
        {Boolean(accuracyRadiusPx) && !isExpired && (
          <div
            ...
            style={{
              width: `${accuracyRadiusPx * 2}px`,
              height: `${accuracyRadiusPx * 2}px`,
            }}
          />
        )}
        {renderPin()}
...
function PinSvg() {
  return (
    <svg className='round-pin' viewBox='0 0 64 64'>
