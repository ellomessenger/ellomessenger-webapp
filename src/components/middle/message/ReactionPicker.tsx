...
const ReactionPicker: FC<OwnProps & StateProps> = ({
  ...
  const renderingPosition = useMemo((): IAnchorPosition | undefined => {
    ...
    return {
      x:
        storedPosition.x +
        (withCustomReactions
          ? FULL_PICKER_SHIFT_DELTA.x
          : LIMITED_PICKER_SHIFT_DELTA.x),
      y:
        storedPosition.y +
        (withCustomReactions
          ? FULL_PICKER_SHIFT_DELTA.y
          : LIMITED_PICKER_SHIFT_DELTA.y),
    };
  ...
  const { positionX, positionY, transformOriginX, transformOriginY, style } =
    useMenuPosition(
      renderingPosition,
      getTriggerElement,
      getRootElement,
  ...
  const selectedReactionIds = useMemo(() => {
    return (message?.reactions?.results || []).reduce<string[]>(
      (acc, { chosenOrder, reaction }) => {
        if (chosenOrder !== undefined) {
          acc.push(
            'emoticon' in reaction ? reaction.emoticon : reaction.documentId
          );
        }
...
export default memo(
  withGlobal<OwnProps>((global): StateProps => {
    ...
    const areSomeReactionsAllowed =
      chatFullInfo?.enabledReactions?.type === 'some';
    const areCustomReactionsAllowed =
      chatFullInfo?.enabledReactions?.type === 'all' &&
    ...
    return {
      ...
      position,
      withCustomReactions:
        chat?.isForbidden || areSomeReactionsAllowed
          ? false
