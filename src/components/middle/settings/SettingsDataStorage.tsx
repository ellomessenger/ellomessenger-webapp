...
const SettingsDataStorage: FC<OwnProps & StateProps> = ({
  ...
  const renderFileSizeCallback = useCallback(
    (value: number) => {
      return lang(
        'AutodownloadSizeLimitUpTo',
        lang('FileSize.MB', String(AUTODOWNLOAD_FILESIZE_MB_LIMITS[value]), 'i')
      );
    },
    [lang]
  );
  ...
  const handleFileSizeChange = useCallback(
    (value: number) => {
      setSettingOption({
        autoLoadFileMaxSizeMb: AUTODOWNLOAD_FILESIZE_MB_LIMITS[value],
      });
    },
    [setSettingOption]
  );

  const handleCanAutoPlayGifsChange = useCallback(
    (value: boolean) => {
  ...
  const handleCanAutoPlayVideosChange = useCallback(
    (value: boolean) => {
  ...
  function renderContentSizeSlider() {
    const value = AUTODOWNLOAD_FILESIZE_MB_LIMITS.indexOf(
      autoLoadFileMaxSizeMb
    );
