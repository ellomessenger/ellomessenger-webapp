...
const URL_ENTITIES = new Set<string>([
...
const useEditing = (
  ...
  useEffectWithPrevDeps(
    ([prevEditedMessage, prevReplyingToId]) => {
      if (!editedMessage) {
        return;
      }
      ...
      // if (
      //   prevEditedMessage?.id === editedMessage.id &&
      //   replyingToId === prevReplyingToId
      // ) {
      //   return;
      // }

      const text =
        !prevEditedMessage && editingDraft?.text.length
          ? editingDraft
  ...
  useEffect(() => {
    ...
    const shouldSetNoWebPage =
      !('webPage' in editedMessage.content) &&
      editedMessage.content.text?.entities?.some((entity) =>
        URL_ENTITIES.has(entity.type)
      );
  ...
  useEffectWithPrevDeps(
    ...
    [
      ...
      getHtml(),
      threadId,
      getShouldResetNoWebPageDebounced,
    ]
  );
